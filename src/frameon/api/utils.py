"""
Core utility functions for data analysis operations.

Provides functions for analyzing join relationships, detecting inconsistent
data mappings, and calculating geographical distances.
"""
...
def analyze_join_keys(
    ...
    if short_result and how not in ["all", "inner", "left", "right", "outer"]:
        raise ValueError(
            "Invalid how. Can be one of ['all', 'inner', 'left', 'right', 'outer']"
        )
    ...
    left_key_str = (
        left_df[left_key].astype(str).agg("|".join, axis=1)
        if len(left_key) > 1
        else left_df[left_key[0]]
    )
    right_key_str = (
        right_df[right_key].astype(str).agg("|".join, axis=1)
        if len(right_key) > 1
        else right_df[right_key[0]]
    )
    ...
    inner_size, left_size, right_size, outer_size = _calculate_join_sizes(
    ...
    result = {
        ...
        "coverage": {
            "left_in_right": (
                len(common_keys) / len(left_unique) if len(left_unique) > 0 else 0
            ),
            "right_in_left": (
                len(common_keys) / len(right_unique) if len(right_unique) > 0 else 0
            ),
            ...
            "left_missing_pct": (
                left_missing / len(left_df) if len(left_df) > 0 else 0
            ),
            "right_missing_pct": (
                right_missing / len(right_df) if len(right_df) > 0 else 0
            ),
        },
        "counts": {
...
def _calculate_join_sizes(
    ...
    common_keys = left_counts.index.intersection(right_counts.index.tolist())
...
def _print_analysis_report(
    ...
    Relationship:
    - Type: {rel["type"]}
    - Left total duplicates (keep=False): {rel["left_total_duplicates"]:,} ({
        format_percent(
            rel["left_total_duplicates"] / results["join_sizes"]["left_rows"]
        )
    } of rows)
    - Right total duplicates (keep=False): {rel["right_total_duplicates"]:,} ({
        format_percent(
            rel["right_total_duplicates"] / results["join_sizes"]["right_rows"]
        )
    } of rows)
    ...
    - Right missing: {cov["right_missing"]:,} ({
        format_percent(cov["right_missing_pct"])
    })
    ...
    if short_result:
        ...
        output.append(
            f"Right table size: {results['join_sizes']['right_rows']:,} rows\n"
        )
        report = pd.DataFrame(
            {
                "Type": f"{rel['type'][-4:-1]}",
        ...
        if how == "inner":
            ...
            report["Right join size"] = f"{results['join_sizes']['right']:,}"
            report["Outer join size"] = f"{results['join_sizes']['outer']:,}"
        display(style_dataframe(report, hide_columns=False, caption="Join info"))
    else:
        print(dedent(report))
...
def find_inconsistent_mappings(
    ...
    if return_summary:
        result = {
            ...
            "inconsistency_rate": (
                len(inconsistent_keys) / len(value_counts)
                if len(value_counts) > 0
                else 0
            ),
            "value_distribution": inconsistent_keys["unique_values"]
            .value_counts()
            .to_dict(),
            "most_inconsistent_keys": inconsistent_keys.nlargest(
                sample_size, "unique_values"
            ).to_dict("index"),
        }
    ...
    if result.empty:
        warnings.warn(
            f"No inconsistent mappings found between {key_column} and {value_column}"
        )
    elif verbose:
...
def haversine_vectorized(
    ...
    if not ((-90 <= lat1[~np.isnan(lat1)]) & (lat1[~np.isnan(lat1)] <= 90)).all():
        raise ValueError(
            "Latitude(s) of first point(s) must be between -90 and 90 degrees"
        )
    if not ((-180 <= lon1[~np.isnan(lon1)]) & (lon1[~np.isnan(lon1)] <= 180)).all():
        raise ValueError(
            "Longitude(s) of first point(s) must be between -180 and 180 degrees"
        )
    if not ((-90 <= lat2[~np.isnan(lat2)]) & (lat2[~np.isnan(lat2)] <= 90)).all():
        raise ValueError(
            "Latitude(s) of second point(s) must be between -90 and 90 degrees"
        )
    if not ((-180 <= lon2[~np.isnan(lon2)]) & (lon2[~np.isnan(lon2)] <= 180)).all():
        raise ValueError(
            "Longitude(s) of second point(s) must be between -180 and 180 degrees"
        )
