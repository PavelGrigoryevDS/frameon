"""
Combined pie and bar chart visualization utilities.

This module provides tools for creating visualizations that combine a pie chart
(showing largest category vs others) with a detailed bar chart. Includes group
size display and flexible aggregation options.
"""
...
def create_pie_bar(
    ...
    if agg_func_for_pie_others not in [
        "mean",
        "median",
        "sum",
        "count",
    ...
    for col in ["x", "y"]:
        if plotly_kwargs.get(col) is not None:
            ...
            if plotly_kwargs[col] not in data_frame.columns:
                raise ValueError(
                    f"Column '{col}' not found in the DataFrame. Available columns: {list(data_frame.columns)}"
                )
    if agg_column:
        ...
        if agg_column not in data_frame.columns:
            raise ValueError(
                f"Column 'agg_column' not found in the DataFrame. Available columns: {list(data_frame.columns)}"
            )
    ...
    df_for_pie = (
        ...
        .assign(
            new_cat_column=lambda df: df[cat_column_axis]
            .astype(str)
            .where(df[cat_column_axis] == max_cat_for_exclude, label_for_others_in_pie)
        )
        .groupby("new_cat_column", observed=observed_for_groupby)
        .agg({num_column: agg_func_for_pie_others, "count_for_show_group_size": "sum"})
        .reset_index()
        .sort_values(num_column, ascending=False)
    )

    # Format group size for display
    df_for_pie["count_for_show_group_size"] = df_for_pie[
        "count_for_show_group_size"
    ].apply(lambda x: f"{x}" if x <= 1000 else ">1000")
    ...
    fig = (
        ...
        .add_traces(
            pie_fig.data, rows=[1] * len(pie_fig.data), cols=[1] * len(pie_fig.data)
        )
        .add_traces(
            bar_fig.data, rows=[1] * len(bar_fig.data), cols=[2] * len(bar_fig.data)
        )
    )
