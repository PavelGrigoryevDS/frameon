...
class TestFrameOnViz:
    ...
    def test_heatmap(self, sample_data):
        ...
        plots = FrameOnViz(sample_data)
        fig = plots.heatmap(
            x="category", y="category2", z="value", do_pivot=True, agg_func="mean"
        )
        assert fig is not None
    ...
    def test_edge_cases(self):
        ...
        # Test constant data
        df = FrameOn({"value": [1] * 20, "category": ["A"] * 10 + ["B"] * 10})
        plots = FrameOnViz(df)
        fig = plots.bar(x="category", y="value")
        assert fig is not None
    ...
    def test_period_change_periods(self, sample_data, period):
        ...
        plots = FrameOnViz(sample_data)
        plots.period_change(metric_col="value", date_col="date", period=period)
    ...
    def test_pairplot_display_modes(self, sample_data, display_mode):
        ...
        plots = FrameOnViz(sample_data)
        plots.pairplot(
            pairs=["value", "value2"], display_mode=display_mode, renderer=None
        )

    @pytest.mark.parametrize(
        "mode", ["base", "dual_hist_trim", "dual_box_trim", "dual_hist_qq"]
    )
    ...
    def test_pairplot_trendlines(self, sample_data, trendline):
        ...
        plots = FrameOnViz(sample_data)
        plots.pairplot(pairs=[("value", "value2")], trendline=trendline, renderer=None)
