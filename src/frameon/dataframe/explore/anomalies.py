"""
Anomaly detection tools for identifying and analyzing data irregularities.
...
class FrameOnAnomaly:
    ...
    def anomalies_report(
        ...
        anomaly_type: Literal[
            "missing", "duplicate", "outlier", "zero", "negative"
        ] = "missing",
        exact: bool = True,
        ...
        if anomaly_type == "outlier":
            if method == "quantile" and not (0 < threshold < 1):
                raise ValueError(
                    "For 'quantile' method, threshold must be between 0 and 1"
                )
        ...
        if show_combinations:
            self.anomalies_combinations(
                n=2, anomaly_type=anomaly_type, method=method, threshold=threshold
            )
    ...
    def anomalies_corr_matrix(
        ...
        p_values = pd.DataFrame(
            np.ones((len(cols), len(cols))), columns=cols, index=cols
        )
        ...
        visual_matrix = pd.DataFrame(index=cols, columns=cols, dtype=object)

        # Apply custom column labels if provided
        display_cols = (
            cols
            if column_labels is None
            else [column_labels.get(col, col) for col in cols]
        )
        ...
        color_matrix = pd.DataFrame(
            np.zeros((len(cols), len(cols))), columns=cols, index=cols
        )
        ...
        for i in range(len(cols)):
            for j in range(i + 1, len(cols)):
                ...
                if p_val < 0.05:
                    color_matrix.iloc[i, j] = (
                        0.1  # Special value for significant p-values
                    )
                else:
        ...
        if (color_matrix == 0.1).any().any():
            fig.add_trace(
                go.Heatmap(
                    z=np.where(
                        (color_matrix == 0.1)
                        & upper_triangle_mask,  # Only the lower triangle
        ...
        if not height:
            height = min(
                max(
                    int(
                        400 + (n_cols - 2) ** 1.1 * 30
                    ),  # Formula: basic size + (number of columns)^1.1 *coefficient
                    min_height,
                ),
                max_height,
            )
        ...
        if not width:
            width = min(
                max(int(500 + (n_cols - 2) * 50), min_width),  # For each column +50px
                max_width,
            )
        # Update layout
        fig.update_layout(
    ...
    def detect_anomalies(
        ...
        if display_count:
            ...
            display(
                style_dataframe(
                    formatted, caption=caption, hide_index=False, hide_columns=False
                )
            )
        ...
        if return_mode:
            ...
            if return_mode == "by_column":
                ...
                anomalies_dict = {}
                for col in columns:
                    mask = self._get_anomaly_mask(
                        [col], anomaly_type, method, threshold
                    )
                    if mask.any():
                ...
                raise ValueError(
                    "Invalid return_mode. Can be one of false, by_column, combined"
                )
    ...
    def anomalies_combinations(
        ...
        if n > len(columns):
            raise ValueError(
                f"Combination size ({n}) cannot exceed number of columns ({len(columns)})"
            )
        ...
        if anomaly_type == "missing":
            ...
            columns = [
                col
                for col in columns
                if self._get_anomaly_mask([col], "outlier", method, threshold).any()
            ]
        elif anomaly_type == "zero":
        ...
        for cols in column_combinations:
            ...
            if co_anomalies > 0:
                results.append({"columns": " | ".join(cols), "count": co_anomalies})

        if not results:
            print(
                f"No co-occurring {anomaly_type} found in any {n}-column combinations"
            )
            return
    ...
    def anomalies_by_categories(
        ...
        if custom_mask is not None:
            ...
            if anomaly_type == "missing":
                ...
                anomaly_mask = self._get_anomaly_mask(
                    None, "outlier", method, threshold
                )
            elif anomaly_type == "zero":
                anomaly_mask = self._get_anomaly_mask(None, "zero")
            elif anomaly_type == "negative":
    ...
    def detect_simultaneous_anomalies(
        ...
        if len(columns) < 2:
            raise ValueError(
                "At least two columns must be specified for simultaneous anomaly detection"
            )
        ...
        if display_report:
            if anomaly_count == 0:
                print(
                    f"No simultaneous {anomaly_type} anomalies found in all specified columns: {columns}"
                )
                return None
    ...
    def anomalies_over_time(
        ...
        labels = pd.Series(
            dict(date="Date", anomaly=f"{anomaly_type.capitalize()} Count")
        )
    ...
    def _get_anomaly_mask(
        ...
        if anomaly_type == "missing":
            ...
            num_columns = (
                self._df[columns].select_dtypes(include=np.number).columns.tolist()
            )
    ...
    def _get_result_for_detect_anomalies(
        ...
        if anomaly_type in ["zero", "negative", "outlier"]:
            columns = [
                col for col in columns if pd.api.types.is_numeric_dtype(self._df[col])
            ]
            if not columns:
        ...
        if anomaly_type == "missing":
            ...
            counts = pd.Series(
                {col: self._df[col].duplicated().sum() for col in columns}
            )
        elif anomaly_type == "outlier":
        ...
        # Prepare results
        results = pd.DataFrame({"Count": counts, "Percent": counts / len(self._df)})
        return results
    ...
    def _get_duplicate_mask(self, exact: bool, normalize_text: bool) -> pd.Series:
        ...
        if normalize_text:
            df_to_check = self._df.apply(
                lambda col: (
                    col.str.lower().str.strip().str.replace(r"\s+", " ", regex=True)
    ...
    def _duplicates_stats(self, exact: bool, normalize_text: bool) -> pd.DataFrame:
        ...
        for keep in keep_options:
            ...
            for exact in exact_options:
                count = self._get_duplicate_mask_with_keep(
                    keep, exact, normalize_text
                ).sum()
    ...
    def _get_duplicate_mask_with_keep(
        self, keep: Literal["first", "last", False], exact: bool, normalize_text: bool
    ) -> pd.Series:
        if exact:
