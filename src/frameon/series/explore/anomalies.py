"""
Series anomaly detection module.

This module provides tools for detecting various types of anomalies in series data,
including outliers, missing values, duplicates, and unusual patterns.
...
class OutlierDetector:
        ...
        **kwargs,
        ...
        if method == OutlierMethod.IQR:
            ...
            model = IsolationForest(
                contamination=contamination, **kwargs.get("iforest_params", {})
            )
            ...
            model = OneClassSVM(nu=contamination, **kwargs.get("svm_params", {}))
            mask = model.fit_predict(values) == -1
    ...
    def prepare_data(
        ...
        df = (
            pd.DataFrame({"time": time_series, "value": series.values})
            .sort_values("time")
            .set_index("time")
        )
        ...
        if resample_freq is not None:
            ...
            df = (
                df.resample(resample_freq).agg({"value": agg_mapping[agg_func]}).ffill()
            )
...
class SeriesOnAnomaly:
    ...
    def anomalies_by_categories(
        ...
        if custom_mask is not None:
            ...
            if anomaly_type == "outlier":
                if method == "quantile" and not (0 < threshold < 1):
                    ...
                    raise ValueError(
                        f"For '{method}' method, threshold must be positive"
                    )
            # Get anomaly mask
        ...
        if anomaly_type == "custom":
            caption = "Custom anomalies distribution across categories"
        else:
    ...
    def anomalies_over_time(
        ...
        if time_column not in parent_df.columns:
            raise ValueError(
                f"Time column '{time_column}' not found in parent DataFrame"
            )
    ...
    def plot_rolling_anomaly_rate(
        ...
        **plotly_kwargs,
        ...
        if not pd.api.types.is_datetime64_any_dtype(
            self._series.parent_df[time_column]
        ):
            raise ValueError(f"Column '{time_column}' must be datetime type")
        ...
        temp_df = pd.DataFrame(
            {"time": time_series, "anomaly": mask.astype(int)}
        ).sort_values("time")
        temp_df["anomaly_rate"] = (
            temp_df["anomaly"]
            .rolling(
                window=window,
            )
        ...
        return self._make_rolling_anomaly_rate_fig(
            anomaly_rate,
            window=window,
    ...
    def _make_rolling_anomaly_rate_fig(
        ...
        fig.update_layout(yaxis_tickformat=".0%", hovermode="x", showlegend=False)
    ...
    def _get_anomaly_mask(
        ...
        if anomaly_type == "missing":
            ...
            if method == "iqr":
                ...
                zscore = np.abs(
                    (self._series - self._series.mean()) / self._series.std()
                )
    ...
    def detect_window_outliers(
        ...
        **kwargs,
        ...
        df = OutlierDetector.prepare_data(
            self._series, time_series, resample_freq, agg_func
        )
        # Apply detection
        if method in [
            OutlierMethod.ISOLATION_FOREST,
        ...
        if show_plot:
            ...
            plot = self._plot_window_results(
                result, method.name, threshold, height, width
            )
    ...
    def detect_outliers(
        ...
        **kwargs,
        ...
        mask, lower, upper = OutlierDetector.detect(
            clean_series.values, method, threshold, contamination, **kwargs
        )

        # Generate outputs
        outliers_df = pd.DataFrame(
            {"value": clean_series[mask], "index": clean_series.index[mask]}
        )

        if show_report:
            report = OutlierDetector.generate_report(
                clean_series, mask, method, threshold, (lower, upper)
            )
            report = pd.DataFrame(report, index=[0])
            caption = f'Outliers in "{self._series.name}"'
            display(
        ...
        if show_plot:
            plot = self._plot_non_window_results(
                clean_series, mask, method.name, (lower, upper), height, width
            )
            plot.show()
        if return_outliers:
    ...
    def _plot_window_results(
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines",
                line=dict(
                    color=colors["normal"], width=1.2, shape="spline", smoothing=1.3
                ),
                opacity=0.9,
        ...
        if "lower_bound" in df.columns:
            fig.add_trace(
                go.Scatter(
                    ...
                    y=df["upper_bound"],
                    name="Upper Bound",
                    line=dict(color=colors["bounds"], dash="dot", width=1.5),
    ...
    def _apply_ml_window(
        self, df: pd.DataFrame, window: str, method: OutlierMethod, **kwargs
    ) -> pd.DataFrame:
    ...
    def _apply_statistical_window(
        ...
        if method == OutlierMethod.CONFIDENCE:
            ...
            result["lower_bound"] = (
                df["value"].rolling(window).quantile(threshold / 100)
            )
            result["upper_bound"] = (
                df["value"].rolling(window).quantile(1 - threshold / 100)
            )
        ...
        result["is_outlier"] = (df["value"] < result["lower_bound"]) | (
