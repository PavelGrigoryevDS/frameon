"""
Pairplot visualization utilities (scatterplot matrices).

This module provides tools for creating pairplots with scatter and density contour
views. Includes correlation calculations, data transformations, and flexible
color coding options for multivariate analysis.
"""
...
class PairplotBuilder:
    ...
    def _validate_params(self) -> None:
        ...
        if self.config.ranges:
            for col, (min_val, max_val) in self.config.ranges.items():
                ...
                if not isinstance(min_val, (float, int, type(None))) or not isinstance(
                    max_val, (float, int, type(None))
                ):
                    raise ValueError(
                        f"Range for '{col}' must be tuple of (min, max) where each can be float or None"
                    )
        ...
        if self.config.transforms:
            if isinstance(self.config.transforms, str):
                ...
                self.config.transforms = {
                    col: self.config.transforms for col in numeric_cols
                }
        ...
        if self.config.display_mode == "density_contour" and (
            self.config.color_mode or self.config.color_column
        ):
        ...
        if self.config.color_mode == "category" and not self.config.color_column:
            raise ValueError(
                "color_column must be specified when color_mode='category'"
            )
        ...
        if (
            self.config.color_mode == "category"
            and self.config.color_column not in self.df.columns
        ):
            raise ValueError(
                f"color_column '{self.config.color_column}' not found in dataframe"
            )
        ...
        if self.config.transforms:
            ...
            for col, transform in self.config.transforms.items():
                ...
                if col in self.config.labels:
                    self.config.labels[col] = (
                        f"{display_name}({self.config.labels[col]})"
                    )
                else:
    ...
    def _create_figure(self) -> None:
        ...
        for i, (col1, col2) in enumerate(self.pairs):
            ...
            if self.config.display_mode == "density_contour":
                self._add_density_contour_trace(
                    df_trim, col1, col2, x_title, y_title, row, col
                )
            else:
                if self.config.color_mode:
                    ...
                    self._add_plain_scatter_trace(
                        df_trim, col1, col2, x_title, y_title, row, col, corr_text
                    )
    ...
    def _generate_plots(self) -> Union[None, Generator[go.Figure, None, None]]:
        ...
        num_pages = (
            num_plots + self.config.plots_per_page - 1
        ) // self.config.plots_per_page
        ...
        for page in range(num_pages):
            ...
            if self.config.rows is not None and self.config.cols is None:
                raise ValueError(
                    "When specifying rows in generator mode, you must also specify cols"
                )
            ...
            if self.config.cols is not None:
                ...
                page_rows = (
                    int(np.ceil(num_plots_page / page_cols))
                    if self.config.rows is None
                    else self.config.rows
                )
            else:
                if num_plots_page == 1:
            ...
            for i, (col1, col2) in enumerate(page_combinations):
                ...
                # Get axis titles
                x_title = (
                    self.config.labels.get(col1, col1) if self.config.labels else col1
                )
                y_title = (
                    self.config.labels.get(col2, col2) if self.config.labels else col2
                )
    ...
    def _add_density_contour_trace(
        self,
        df: pd.DataFrame,
        col1: str,
        col2: str,
    ...
    def _add_colored_scatter_trace(
        ...
        if self.config.color_mode == "category":
            ...
            if self.config.color_mode == "count":
                ...
                color_data = df.groupby(["_x_sector_", "_y_sector_"], observed=False)[
                    col1
                ].transform("count")
            else:  # 'kde'
        ...
        if not (trace_idx == 0):
            ...
            self.figure.update_traces(
                colorbar=dict(title=color_title), selector=dict(row=row, col=col)
            )

        return max_density

    def _add_plain_scatter_trace(
        self,
        df: pd.DataFrame,
        col1: str,
        col2: str,
    ...
    def _update_axes(self, x_title: str, y_title: str, row: int, col: int) -> None:
        ...
        self.figure.update_xaxes(
            title_text=x_title, title_font=dict(size=12), row=row, col=col
        )
        self.figure.update_yaxes(
            title_text=y_title, title_font=dict(size=12), row=row, col=col
        )
    ...
    def _finalize_figure_layout(self, max_density: Optional[float] = None) -> None:
        ...
        self.figure.update_yaxes(title_standoff=5)
        self.figure.update_xaxes(title_standoff=5)
        layout_updates = dict(
            height=self.config.height,
            width=self.config.width,
        ...
        if self.config.color_mode == "category":
            if not self.config.show_legend_title:
                layout_updates.update(
                    {"legend_title_text": "", "margin": dict(l=50, r=50, t=80, b=50)}
                )
