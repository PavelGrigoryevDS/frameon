...
class TestSeriesOnAnomaly:
    ...
    def test_zscore_method(self, normal_series):
        ...
        detector = SeriesOnAnomaly(normal_series)
        result = detector.detect_outliers(method=OutlierMethod.ZSCORE, return_outliers=True)
        assert not result.empty
        
    # Window Detection Tests
    def test_window_outliers(self, df_with_time_column):
        """Test window-based outlier detection"""
        result = df_with_time_column['missing1'].explore.detect_window_outliers(time_column='date', window=7)
    ...
    @pytest.mark.parametrize("method", ['iqr', 'zscore', 'quantile', 'mad', 'tukey', 'isolation_forest', 'lof', 'one_class_svm'])
    ...
    @pytest.mark.parametrize("method", ['confidence', 'iqr', 'zscore', 'quantile', 'mad', 'tukey', 'isolation_forest', 'lof', 'one_class_svm'])
    def test_window_outliers_methods(self, method, df_with_time_column):
        """Test different window-based outlier detection methods"""
        result = df_with_time_column['value'].explore.detect_window_outliers(time_column='date', window=7, method=method)
        assert result is not None
