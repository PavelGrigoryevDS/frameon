"""
Period-over-period change visualization utilities.

This module provides tools for visualizing changes across time periods (MoM, WoW, DoD, YoY)
with automatic coloring of positive/negative changes and flexible aggregation options.
"""
...
def period_change(
    ...
    if color_dict is None:
        color_dict = {"positive": "#57B16E", "negative": "#E25559"}
    ...
    freq_map = {"mom": "ME", "wow": "W", "dod": "D", "yoy": "YE"}
    ...
    # Aggregate data
    df_agg = df.groupby(group_cols_with_grouper, as_index=False)[metric_col].agg(
        agg_func
    )
    ...
    if period == "wow":
        ...
        date_range = pd.date_range(
            min_date - pd.Timedelta(days=min_date.weekday()), max_date, freq=freq
        )
    else:
    ...
    df_agg = (
        df_agg.set_index(["period"] + group_cols)
    ...
    df_agg["change_direction"] = np.where(
        df_agg[change_col] >= 0, "positive", "negative"
    )
    ...
    fig.update_layout(yaxis_zeroline=True, yaxis_tickformat=".0%", showlegend=False)
