"""
Core base classes extending pandas functionality.

Provides FrameOn and SeriesOn classes that enhance pandas DataFrame and Series
with additional methods while maintaining full compatibility.
"""
...
class SeriesOn(pd.Series):
    ...
    def __dir__(self):
        return sorted(set(super().__dir__() + ["explore", "preproc", "parent_df"]))
...
class FrameOn(pd.DataFrame):
    ...
    def __init__(
        self, data=None, index=None, columns=None, dtype=None, copy=False, name_df=None
    ):
        """
    ...
    def _constructor_sliced(self):
        ...
        def _smart_constructor_sliced(
            data=None, index=None, name=None, dtype=None, copy=False
        ):
            return SeriesOn(
                data=data,
                index=index,
