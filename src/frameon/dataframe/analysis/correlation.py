"""
Correlation analysis functionality for exploring relationships between variables.
...
class CorrelationAnalyzer:
    ...
    def corr_matrix(
            ...
            for j, col2 in enumerate(cols):
                if i < j:
                    ...
                    if len(x) < 2:
                        ...
                        _, p_val = CorrelationAnalyzer._calculate_correlation(
                            method, x, y
                        )
        ...
        for i in range(1, len(cols)):
            for j in range(i):
                visual_matrix.iloc[i, j] = (
                    f"{corr_matrix.iloc[i, j]:.{decimal_places_coef}f}"
                )
