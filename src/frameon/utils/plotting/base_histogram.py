"""
Histogram visualization utilities with multiple view modes.

This module provides tools for creating histograms with various configurations
including KDE overlays, box plots, and QQ plots. Supports quantile filtering
and dual-view comparisons for data analysis.
"""
...
class HistogramBuilder:
    ...
    def build(self, **kwargs) -> Union[None, CustomFigure]:
                ...
                if key in [
                    ...
                    if value is not None:
                        if isinstance(value, str):
                            raise ValueError(
                                f"Unsupported type for '{key}'. "
                                "If 'data' is not provided, '{key}' must be Series, array-like, or list."
                            )
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.config.mode != "base" and any(
            self.plotly_kwargs.get(k)
            for k in ["facet_col", "facet_row", "animation_frame"]
        ):
            raise ValueError(
                "Facet and animation features are only supported in 'base' mode"
            )
        ...
        if (
            self.config.mode in ["dual_hist_trim", "dual_box_trim"]
            and self.config.upper_quantile == 1
            and self.config.lower_quantile == 0
        ):
            raise ValueError(
                f"At least one quantile must differ from 0 (lower) or 1 (upper) for {self.config.mode}."
            )
    ...
    def _preprocess_data(self) -> None:
        ...
        if group_cols:
            ...
            mask = df.groupby(group_cols, observed=False)[num_col].transform(
                filter_group
            )
    ...
    def _create_base_figure(self) -> None:
        ...
        if (
            not self.config.show_hist
            and not self.config.show_kde
            and not self.config.show_box
        ):
        ...
        cols = (
            len(fig._grid_ref[0])
            if hasattr(fig, "_grid_ref") and len(fig._grid_ref) > 0
            else 1
        )
    ...
    def _create_dual_histogram_view(self) -> None:
        ...
        cols = (
            len(subplots._grid_ref[0])
            if hasattr(subplots, "_grid_ref") and len(subplots._grid_ref) > 0
            else 1
        )
        # Create subplots figure
    ...
    def _create_box_histogram_view(self) -> None:
        ...
        cols = (
            len(fig_hist._grid_ref[0])
            if hasattr(fig_hist, "_grid_ref") and len(fig_hist._grid_ref) > 0
            else 1
        )
        # Create subplots figure
    ...
    def _make_box_histogram_layout(self) -> go.Figure:
        ...
        if self.config.show_box:
            ...
            return make_subplots(
                ...
                shared_xaxes=True,
                specs=[[{"rowspan": 2}, {"colspan": 1}], [None, {"colspan": 1}]],
            )
    ...
    def _add_box_histogram_traces(self, fig, left_fig, right_fig) -> None:
        ...
        for trace in left_fig.data:
            ...
            trace.showlegend = (
                False
                if self.config.show_kde or self.config.show_hist or self.config.show_box
                else True
            )
        ...
        for trace in right_fig.data:
            if trace.type == "box" and self.config.show_box:
                ...
                fig.add_trace(trace, row=2 if self.config.show_box else 1, col=2)
    ...
    def _add_qq_histogram_traces(self, fig, left_fig, right_fig) -> None:
        ...
        for trace in left_fig.data:
            if trace.type == "box" and self.config.show_box:
                ...
                fig.add_trace(trace, row=2 if self.config.show_box else 1, col=1)
        # Add qqplot
        for trace in right_fig.data:
            fig.add_trace(trace, row=1, col=2)
    ...
    def _style_qq_histogram_view(self, fig) -> None:
        ...
        if self.config.show_box:
            fig.update_xaxes(
                showticklabels=False, showline=False, ticks="", row=1, col=1
            )
    ...
    def _make_subplots_figure(
        self, traces: List[go.Histogram], rows: int, cols: int
    ) -> go.Figure:
    ...
    def _calculate_row_heights(self) -> Tuple[float, float]:
        ...
        if "color" in self.plotly_kwargs:
            group_labels = self.plotly_kwargs["data_frame"][
                self.plotly_kwargs["color"]
            ].unique()
            ...
            height_increase = (
                categories_cnt * 20
            )  # Increase height by 20 pixels for each box plot
            base_height = 400
    ...
    def _add_traces_to_subplots(
        self, fig: go.Figure, traces: List[go.Histogram], rows, cols
    ):
        ...
        for trace in traces:
            ...
            if col == 1:
                fig.update_yaxes(title=self.yaxis_title, row=hist_row, col=col)
            else:
                if not mode == "dual_hist_trim":
                    fig.update_yaxes(showticklabels=False, row=hist_row, col=col)
            if hist_row == last_row:
                fig.update_xaxes(title=self.xaxis_title, row=hist_row, col=col)
            # Add boxplot if enabled
            if show_box:
                self._add_boxplot_trace(fig, trace, hist_row, col)

    def _add_boxplot_trace(
        self, fig: go.Figure, hist_trace: go.Histogram, hist_row: int, col: int
    ):
    ...
    def _get_row_col_from_axis(self, axis_name, cols) -> Tuple[int, int]:
        ...
        if axis_name == "x":
            ...
            axis_number = (
                int(axis_name.replace("x", "")) if axis_name.startswith("x") else 1
            )
    ...
    def _create_kde_trace(self, hist_trace: go.Histogram) -> Union[go.Scatter, None]:
        ...
        if len(hist_data) < 2:
            warnings.warn(
                f'In group "{label_for_kde}" number of elements less then 2. KDE line cannot be constructed.'
            )
            x_range = [0]
            y_values = [0]
        else:
    ...
    def _apply_final_styling(self) -> None:
        ...
        if self.plotly_kwargs.get("color"):
            ...
            if self.config.legend_position == "top":
                update_config.update({"margin": dict(l=50, r=50, t=70, b=50)})
        ...
        if self.config.xaxis_type == "category":
            ...
            if x_data is not None:
                ...
                self.figure.update_xaxes(
                    ...
                    tickvals=x_data,
                    ticktext=[
                        f"{float(val):.2f}" if val % 1 != 0 else int(val)
                        for val in x_data
                    ],
                )
        elif self.config.xaxis_type:
            self.figure.update_xaxes(type=self.config.xaxis_type)
        if self.config.yaxis_type == "category":
            ...
            self.figure.update_yaxes(type=self.config.yaxis_type)
