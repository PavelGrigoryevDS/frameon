...
class RFMAnalyzer:
    ...
    def _validate_rfm_parameters(self) -> None:
        ...
        missing_columns = [
            col_name
            for param_name, col_name in required_columns.items()
    ...
    def _prepare_rfm_data(self) -> None:
        ...
        self.df_rfm = (
            ...
            .agg(
                ...
                frequency=(
                    config.order_id_col if config.order_id_col else config.date_col,
                    "nunique",
                ),
    ...
    def _calculate_rfm_scores(self) -> None:
        ...
        if len(df_rfm) == 1:
            ...
            df_rfm["rfm_score"] = (
                f"{config.score_bins}{config.score_bins}{config.score_bins}"
            )
            return
    ...
    def _assign_segments(self) -> None:
        ...
        df_rfm["rfm_segment"] = df_rfm["rfm_score"].replace(
            self.segment_names, regex=True
        )
    ...
    def _create_visualizations(self) -> go.Figure:
        ...
        if invalid_viz:
            raise ValueError(
                f"Invalid visualization types: {invalid_viz}. "
                f"Available options: {list(available_viz.keys())}"
            )

        return {
            name: fn() for name, fn in available_viz.items() if name in config.plots
        }
    ...
    def _make_histograms(self) -> go.Figure:
        ...
        trim_data = (
            hasattr(self.config_rfm, "lower_quantile")
            and hasattr(self.config_rfm, "upper_quantile")
            and (
                self.config_rfm.lower_quantile > 0 or self.config_rfm.upper_quantile < 1
            )
        )
        ...
        if trim_data:
            ...
            trim_info = (
                "<br><sup>Top row: Original distributions | Bottom row: Trimmed: "
            )
        ...
        if trim_data:
            ...
            fig.update_layout(
                height=350,
                width=900,
                title_text="RFM Score Distributions",
            )
        return fig
    ...
    def _make_heatmap(self) -> go.Figure:
        ...
        heatmap = px.imshow(
            ...
            labels=dict(x="Frequency Score", y="Recency Score", color="Monetary"),
            aspect="auto",
        )
    ...
    def _make_pairwise_heatmaps(self) -> go.Figure:
        ...
        for i, (x, y) in enumerate(pairs, 1):
            ...
            heatmap_data = (
                df_rfm.groupby([x, y], observed=False).size().unstack(fill_value=0)
            )
            ...
            fig.update_xaxes(
                title_text=y.replace("_score", "").title(),
                type="category",
                title_standoff=5,
                row=1,
                col=i,
            ...
            fig.update_yaxes(
                title_text=x.replace("_score", "").title(),
                type="category",
                title_standoff=5,
                row=1,
                col=i,
        ...
        fig.update_layout(
            coloraxis=dict(
                colorscale=config.color_continuous_scale, colorbar_title_text="Count"
            ),
            height=330,
            width=900,
    ...
    def _make_pairwise_heatmaps_by_score(self) -> go.Figure:
        ...
        for score_val in range(1, config.score_bins + 1):
            ...
            heatmap_data = (
                filtered_df.groupby([y, x], observed=False).size().unstack(fill_value=0)
            )
        ...
        # Create subplot grid
        name_map = {"recency_score": "R", "frequency_score": "F", "monetary_score": "M"}
        name_x = name_map[x]
        name_y = name_map[y]
        ...
        n_rows = (
            config.score_bins + 2
        ) // 3  # This gives 1 row for 3 bins, 2 rows for 5 bins
        ...
        for score_val in range(1, config.score_bins + 1):
            subplot_titles.append(
                f"{name_x} vs {name_y} ({name_slice_var}={score_val})"
            )
        ...
        for score_val in range(1, config.score_bins + 1):
            ...
            fig.add_trace(heatmap.data[0], row=row_idx, col=col_idx)
    ...
    def _make_pairwise_bar_by_score(self) -> go.Figure:
        ...
        grouped_df = (
            df_rfm.groupby(
                ["recency_score", "frequency_score", "monetary_score"], observed=False
            )
            .size()
        ...
        fig.update_layout(
            height=350
            * ((config.score_bins + 2) // 3),  # Adjust height based on number of rows
            width=1100,
            margin=dict(t=100),
            showlegend=True,
            title_y=0.97,
    ...
    def _make_segment_bar(self) -> go.Figure:
        ...
        bar.update_layout(title="Customer Segments Distribution", height=400, width=600)
        return bar
    ...
    def _make_pairwise_scatters_by_score(self) -> go.Figure:
        ...
        for score_val in range(config.score_bins, 0, -1):
            for x, y in [
                ...
                third_var = next(
                    v for v in ["recency", "frequency", "monetary"] if v not in (x, y)
                )
                pair_name = f"{name_map[x]} vs {name_map[y]}"
                subplot_titles.append(
                    f"{pair_name} ({name_map[third_var]}={score_val})"
                )
        ...
        for score_var in ["recency_score", "frequency_score", "monetary_score"]:
            for score_val in range(1, config.score_bins + 1):
                ...
                for x, y in [
                    ...
                    third_var = next(
                        v
                        for v in ["recency", "frequency", "monetary"]
                        if v not in (x, y)
                    )
                    ...
                    # Add the trace
                    fig.add_trace(scatter.data[0], row=row_idx, col=col_idx)
    ...
    def _make_raw_distribution_grid(self) -> go.Figure:
        ...
        for row, raw_var in enumerate(["recency", "frequency", "monetary"], 1):
            ...
            score_vars = [
                v
                for v in ["recency_score", "frequency_score", "monetary_score"]
            ...
            # Y-axis labels
            fig.update_yaxes(
                title_text=name_map[raw_var], title_standoff=5, row=row, col=1
            )

            # X-axis labels
            fig.update_xaxes(
                title_text=name_map[score_vars[0]], title_standoff=5, row=row, col=1
            )
            fig.update_xaxes(
                title_text=name_map[score_vars[1]], title_standoff=5, row=row, col=2
            )
        for score_val in range(1, config.score_bins + 1):
