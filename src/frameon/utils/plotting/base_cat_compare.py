"""
Categorical data comparison visualization utilities.

This module provides tools for comparing two categorical variables through
multiple visualization types including bar charts and cross-tabulations.
Supports data normalization, category trimming, and flexible layout configurations.
"""
...
class CatCompareBuilder:
    ...
    def build(self, **kwargs) -> Union[List[CustomFigure], None]:
        ...
        self.default_titles = self._generate_section_titles(
            self.cat1, self.cat2, self.config.labels
        )
        self._prepare_data()
    ...
    def _set_default_params_in_plotly_kwargs(self) -> None:
        ...
        if "data_for_px_bar" not in self.config.hover_data and isinstance(
            self.config.hover_data, dict
        ):
    ...
    def _prepare_data(self) -> None:
        ...
        self.df_cat1 = (
            self.data_frame.groupby(self.data_frame[self.cat1], observed=True)
            .size()
            .to_frame("value_for_px_bar")
        )
        self.df_cat1.index = self.df_cat1.index
        self.df_cat2 = (
            self.data_frame.groupby(self.data_frame[self.cat2], observed=True)
            .size()
            .to_frame("value_for_px_bar")
        )
        self.df_cat2.index = self.df_cat2.index
        self.df_cat1_by_cat2 = pd.crosstab(
            self.data_frame[self.cat1], self.data_frame[self.cat2]
        )
        self.df_cat2_by_cat1 = self.df_cat1_by_cat2.T
        if self.config.trim_top_n_cat1:
            self.top_n_cat1 = (
                self.df_cat1["value_for_px_bar"]
                .nlargest(self.config.trim_top_n_cat1)
                .index.tolist()
            )
        if self.config.trim_top_n_cat2:
            self.top_n_cat2 = (
                self.df_cat2["value_for_px_bar"]
                .nlargest(self.config.trim_top_n_cat2)
                .index.tolist()
            )
            ...
            if isinstance(order_spec, str):
                ...
                if "category" in order_spec:
                    ...
                    processed_orders[col] = sorted(
                        categories, key=lambda x: str(x), reverse=not ascending
                    )
                else:
                    # Count-based sorting
                    ascending = "ascending" in order_spec
                    counts = self.data_frame[col].value_counts()
                    processed_orders[col] = (
                        counts.sort_values(ascending=ascending)
                        .index.astype(str)
                        .tolist()
                    )
    ...
    def _create_figures(self) -> None:
        ...
        row_sets = [
            ...
            [
                (self.df_cat1_by_cat2, "cat1_by_cat2", "index"),
                (self.df_cat1_by_cat2, "cat1_by_cat2", "columns"),
            ],  # Row 2
            [
                (self.df_cat2_by_cat1, "cat2_by_cat1", "index"),
                (self.df_cat2_by_cat1, "cat2_by_cat1", "columns"),
            ],  # Row 3
        ]
    ...
    def _prepare_df_for_visualization(
        self, df: pd.DataFrame, graph_type: str, norm_mode: str
    ) -> pd.DataFrame:
        ...
        result = pd.concat(
            [normalized, df], axis=1, keys=["data_for_px_bar", "customdata_for_px_bar"]
        )
        ...
        if graph_type in ["cat1", "cat2"]:
            ...
            result_long = result.melt(
                id_vars=cat_col, var_name="type", value_name="value_for_px_bar"
            )
            result_long[cat_col] = result_long[cat_col].astype(str)
        else:
            new_columns = result.columns.map(
                lambda x: f"{x[0]}__{x[1]}" if x[0] not in [cat1, cat2] else x[0]
            ).tolist()
            ...
            result_long[["type", cat2 if graph_type == "cat1_by_cat2" else cat1]] = (
                result_long[
                    f"type_{cat2 if graph_type == 'cat1_by_cat2' else cat1}"
                ].str.split("__", n=1, expand=True)
            )
            result_long = result_long.drop(
                columns=f"type_{cat2 if graph_type == 'cat1_by_cat2' else cat1}"
            )
            result_long[cat1] = result_long[cat1].astype(str)
            result_long[cat2] = result_long[cat2].astype(str)

        # Transform to final format
        result_long = result_long.set_index(
            list(set(result_long.columns) - set(["value_for_px_bar"]))
        )
        result_long = (
            result_long.unstack(level="type").droplevel(0, axis=1).reset_index()
        )
        ...
        if graph_type == "cat1":
            return {
                **base_kwargs,
                "y": self.cat1,
                "x": "data_for_px_bar",
                "custom_data": "customdata_for_px_bar",
            }
    ...
    def _update_subplot_axes(
        self, fig: go.Figure, sub_fig: go.Figure, col: int, row_index: int
    ):
        ...
        if row_index == 0:
            ...
            fig.update_yaxes(
                ...
                categoryorder="array",
                categoryarray=(
                    categoryarray[::-1] if categoryarray else None
                ),  # For horisontal bar reversed order
                row=1,
                col=col,
            )
        else:
            fig.update_yaxes(showgrid=False, title_standoff=7)
            fig.update_yaxes(title=None, row=1, col=2)
            fig.update_xaxes(showgrid=True, title_standoff=7)
            if col == 2:
    ...
    def _apply_figure_layout(self, fig: go.Figure, row_index: int):
        ...
        fig.update_layout(
            height=self.config.heights[row_index], width=self.config.width
        )
        if row_index in [1, 2]:
            legend_titles = [
            ...
            for x_pos, text in [
                (0.25, "Normalized by axis"),
                (0.75, "Normalized by legend"),
            ]:
                fig.add_annotation(
                    text=text,
                    xref="paper",
        ...
        fig.update_layout(title_text=self.default_titles[row_index])
