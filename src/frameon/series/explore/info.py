"""
Series information and analysis module.

This module provides tools for summarizing and analyzing series data,
including statistics for numeric, categorical, datetime and text data.
Supports both tabular summaries and visualizations.
"""
...
class SeriesOnInfo:
    ...
    def info(
        ...
        column_type: Optional[
            Literal["numeric", "categorical", "text", "datetime"]
        ] = None,
        ...
        **plotly_kwargs,
        ...
        column_type : str, optional
            Type for column, if None type detect automaticaly.
            Can be one of 'numeric', 'categorical', 'text', 'datetime'.
        ...
        if column_type and column_type not in [
            "numeric",
            "categorical",
            "text",
            "datetime",
            ...
            raise ValueError(
                "column_type must be on of 'numeric', 'categorical', 'text', 'datetime'"
            )
        ...
        if column_type not in ["datetime", "text", "categorical"] and (
            is_numeric or column_type == "numeric"
        ):
            ...
            if plot:
                if hist_mode not in ["base", "dual_hist_trim", "dual_hist_qq"]:
                    raise ValueError(
                        "hist_mode must be on of 'base', dual_hist_trim' or 'dual_hist_qq'"
                    )
        ...
        if column_type != "datetime" and (
            column_type in ["text", "categorical"]
            or pd.api.types.is_string_dtype(self._series.dropna())
        ):
            if pd.api.types.is_datetime64_any_dtype(self._series):
                raise ValueError(
                    "Column cannot be datetime when processing as text/categorical"
                )
            ...
            is_categegory = column_type != "text" and (
                column_type == "categorical" or is_categorical_column(self._series)
            )
            summary = self._generate_summary_for_categorical(is_categegory)
            display(summary)
            if plot:
                fig = self._generate_combined_charts(
                    is_categegory,
                    top_n,
                    max_words,
                    title,
    ...
    def _generate_basic_stats_datetime(self) -> pd.DataFrame:
        ...
        timedeltas = (
            valid_dates.sort_values().diff().dropna()
        ...
        stats = {
            ...
            "Avg Days Frequency": (
                str(round(timedeltas.mean().total_seconds() / (24 * 3600), 2))
                if not timedeltas.empty
                else "N/A"
            ),
            "Min Days Interval": (
                timedeltas.min().days if not timedeltas.empty else "N/A"
            ),
            "Max Days Interval": (
                timedeltas.max().days if not timedeltas.empty else "N/A"
            ),
            "Memory Usage": ram,
        }
    ...
    def _generate_data_quality_stats_datetime(self) -> pd.DataFrame:
        ...
        formatted_zeros = (
            format_count_with_percentage(zeros, len_column) if zeros > 0 else "---"
        )
        missings = self._series.isna().sum()
        formatted_missings = (
            format_count_with_percentage(missings, len_column)
            if missings > 0
            else "---"
        )
        distinct = valid_dates.nunique()
        formatted_distinct = (
            format_count_with_percentage(distinct, len_column)
            if distinct > 0
            else "---"
        )
        duplicates = self._series.duplicated().sum()
        formatted_duplicates = (
            format_count_with_percentage(duplicates, len_column)
            if duplicates > 0
            else "---"
        )
        dup_values = self._series.value_counts()[
            self._series.value_counts() > 1
        ].count()
        formatted_dup_values = (
            format_count_with_percentage(dup_values, len_column)
            if dup_values > 0
            else "---"
        )
        stats = {
    ...
    def _generate_temporal_stats_datetime(self) -> pd.DataFrame:
        ...
        if not valid_dates.empty:
            ...
            stats = {
                "Missing Years": (
                    format_count_with_percentage(missing_years, all_years)
                    if missing_years > 0
                    else "---"
                ),
                "Missing Months": (
                    format_count_with_percentage(missing_months, all_months)
                    if missing_months > 0
                    else "---"
                ),
                "Missing Weeks": (
                    format_count_with_percentage(missing_weeks, all_weeks)
                    if missing_weeks > 0
                    else "---"
                ),
                "Missing Days": (
                    format_count_with_percentage(missing_days, all_days)
                    if missing_days > 0
                    else "---"
                ),
                "Weekend Percentage": f"{valid_dates.dt.weekday.isin([5, 6]).mean():.1%}",
                "Most Common Weekday": (
                    valid_dates.dt.day_name().mode()[0]
                    if not valid_dates.empty
                    else "N/A"
                ),
            }
    ...
    def _generate_full_datetime_summary(self) -> pd.DataFrame:
        ...
        full_summary = pd.concat(
            [basic_stats, data_quality_stats, temporal_stats], axis=1
        )
        ...
        full_summary.columns = pd.MultiIndex.from_tuples(
            [
                ("Summary", "Metric"),
                ("Summary", "Value"),
                ("Data Quality Stats", "Metric"),
                ("Data Quality Stats", "Value"),
                ("Temporal Stats", "Metric"),
                ("Temporal Stats", "Value"),
            ]
        )
    ...
    def _generate_summary_for_numeric(self) -> pd.DataFrame:
        ...
        missing = (
            format_count_with_percentage(missing, len_column) if missing else "---"
        )
        ...
        values_with_duplicates = self._series.value_counts()[
            self._series.value_counts() > 1
        ].count()
        values_with_duplicates = (
            format_count_with_percentage(values_with_duplicates, len_column)
            if duplicates != "---"
            else "---"
        )
        ...
        negative = (
            format_count_with_percentage(negative, len_column)
            if negative > 0
            else "---"
        )
        ...
        infinite = (
            format_count_with_percentage(infinite, len_column)
            if infinite > 0
            else "---"
        )
    ...
    def _generate_value_counts_for_numeric(self, top_n: int = 9) -> pd.DataFrame:
        ...
        df_res["Value"] = df_res["Value"].apply(
            lambda x: f"{x:.2f}" if x % 1 != 0 else f"{x:.0f}"
        )
        # Format the Percent column to show "<1%" for values less than 1
        df_res["Count"] = df_res["Count"].apply(
            lambda x: format_count_with_percentage(x, len_column)
        )
    ...
    def _combine_numeric_stats(self, *stats_dfs: pd.DataFrame) -> pd.DataFrame:
        ...
        for i, df in enumerate(stats_dfs):
            section_name = ["Summary", "Percentiles", "Detailed Stats", "Value Counts"][
                i
            ]
    ...
    def _generate_summary_for_categorical(self, is_categegory: bool) -> pd.DataFrame:
        ...
        quality_metrics = {
            ...
            "Missing Values": (
                format_count_with_percentage(missing, total) if missing else "---"
            ),
            "Empty Strings": (
                format_count_with_percentage(empty, total) if empty else "---"
            ),
            "Distinct Values": (
                format_count_with_percentage(distinct, total) if distinct else "---"
            ),
            "Non-Duplicates": (
                format_count_with_percentage(unique_once, total)
                if unique_once
                else "---"
            ),
            "Exact Duplicates": (
                format_count_with_percentage(exact_duplicates, total)
                if exact_duplicates
                else "---"
            ),
            "Fuzzy Duplicates": (
                format_count_with_percentage(fuzzy_duplicates, total)
                if fuzzy_duplicates
                else "---"
            ),
            "Values with Duplicates": (
                format_count_with_percentage(values_with_duplicates, total)
                if values_with_duplicates
                else "---"
            ),
            "Memory Usage": ram,
        }
        text_metrics = {
            ...
            "Median Length (chars)": (
                f"{median_len:.1f}" if not pd.isna(median_len) else "N/A"
            ),
        ...
        quality_metrics_df = pd.DataFrame(
            quality_metrics.items(), columns=["Metric", "Value"]
        )
        text_metrics_df = pd.DataFrame(
            text_metrics.items(), columns=["Metric", "Value"]
        )
        ...
        if is_categegory:
            full_summary = pd.concat(
                [quality_metrics_df, text_metrics_df, value_counts_df], axis=1
            )
            full_summary.columns = pd.MultiIndex.from_tuples(
                [
                    ("Summary", "Metric"),
    ...
    def _generate_value_counts_for_categorical(self, top_n: int = 9) -> pd.DataFrame:
        ...
        df_result["Count"] = df_result.apply(
            lambda row: format_count_with_percentage(row["Count"], total_count), axis=1
        )
    ...
    def _generate_combined_charts(
        ...
        if is_categegory:
            ...
            fig = create_wordcloud_plotly(
                text, max_words=max_words, width=width, height=height, return_fig=True
            )
        return CustomFigure(fig)
