...
class TestStatisticalTests:
    ...
    def test_ordered_model(self):
        """Test ordered model"""
        data = {
            'y': ['bad', 'medium', 'good', 'medium', 'good', 'bad', 'medium', 'good', 'good', 'medium'], 
            'x1': [0.5, 1.2, 2.3, 1.0, 2.1, 0.8, 1.5, 2.4, 2.2, 1.3],
            'x2': [10, 15, 20, 12, 18, 11, 16, 22, 19, 14]
        }
        df = pd.DataFrame(data)

        df['y'] = pd.Categorical(df['y'], categories=['bad', 'medium', 'good'], ordered=True)
        stats = StatisticalTests(df)
    ...
    def test_exception_handling(self, sample_data):
        ...
        with patch('sklearn.metrics.accuracy_score', side_effect=Exception("Test error")):
            stats.feature_importance_analysis(
