"""
Miscellaneous utility functions for the frameon package.

Provides various helper functions including:
- Data validation and type checking
- DataFrame formatting and styling
...
def is_categorical_column(column: pd.Series) -> bool:
    ...
    return (
        not pd.api.types.is_datetime64_any_dtype(column)
        and not pd.api.types.is_numeric_dtype(column)
        and (column.nunique() / len(column) < 0.2)
...
def is_text_column(column: pd.Series) -> bool:
    ...
    return pd.api.types.is_string_dtype(column.dropna()) and not is_categorical_column(
        column
    )
...
def format_count_with_percentage(count: int, total: int, precision: int = 0) -> str:
    """
...
def analyze_anomalies_all_categories(
    ...
    if exclude_columns is not None:
        categorical_cols = [
            col for col in categorical_cols if col not in exclude_columns
        ]
    ...
    for col in categorical_cols:
        try:
            ...
            result = pd.merge(category_stats, anomaly_stats, on=col, how="left")
            result[["anomaly_count", "category_count"]] = result[
                ["anomaly_count", "category_count"]
            ].fillna(0)
            ...
            result["anomaly_rate"] = (
                result["anomaly_count"] / result["category_count"] * 100
            )
            ...
            if not result.empty:
                ...
                result["category_count"] = (
                    result["category_count"].astype(int).astype(str)
                )
                result["anomaly_count"] = (
                    result["anomaly_count"].astype(int).astype(str)
                )

                # Format percentages with 2 decimal places
                result["category_pct"] = (
                    result["category_pct"].round(1).astype(str) + "%"
                )
                result["anomaly_pct"] = result["anomaly_pct"].round(1).astype(str) + "%"
                result["pct_diff"] = result["pct_diff"]
                result["anomaly_rate"] = (
                    result["anomaly_rate"].round(1).astype(str) + "%"
                )
...
def restore_full_index(
    ...
    if date_col not in df.columns:
        raise ValueError(
            f"Column '{date_col}' not found in the DataFrame. Available columns: {list(df.columns)}"
        )
    ...
    for col in group_cols:
        if col not in df.columns:
            raise ValueError(
                f"Column '{col}' not found in the DataFrame. Available columns: {list(df.columns)}"
            )
    ...
    df = (
        df.set_index([date_col] + group_cols)
...
def convert_to_dataframe(
    ...
    **kwargs : dict
    ...
    if x is not None:
        if isinstance(x, pd.Series):
            ...
            if data is not None and x in data.columns:
                ...
                raise ValueError(
                    "Unsupported type for 'x'. Must be a str (column name in data), Series, or array-like."
                )
        else:
            raise ValueError(
                "Unsupported type for 'x'. Must be a str (column name in data), Series, or array-like."
    ...
    if y is not None:
        if isinstance(y, pd.Series):
            ...
            if data is not None and y in data.columns:
                ...
                raise ValueError(
                    "Unsupported type for 'y'. Must be a str (column name in data), Series, or array-like."
                )
        else:
            raise ValueError(
                "Unsupported type for 'y'. Must be a str (column name in data), Series, or array-like."
    ...
    if data is None:
        raise ValueError(
            "Either 'data' or 'x/y' data must be provided. "
            "Supported types for 'data' are DataFrame, Series, array-like, or dict."
        )
