"""
Customer segmentation analysis tools for exploring and visualizing segments.

Provides functionality for:
- Segment profiling and comparison
- Polar chart visualizations
- Interactive segment tables
- Metric analysis across dimensions
"""
...
class SegmentAnalyzer:
    ...
    def segment_polar(
        ...
        missing_columns = [
            col
            for col in [dimension, count_column] + metrics
        ...
        segment_counts = (
            plot_data.groupby(dimension, observed=True)[count_column]
            .count()
            .reset_index()
        )
        ...
        category_orders = {
            dimension: segment_counts.sort_values("Count", ascending=False)[
                dimension
            ].tolist()
        }
        ...
        if agg_func in ["median", "mean", "sum", "min", "max"]:
            segment_stats = plot_data.groupby(dimension, observed=True)[metrics].agg(
                agg_func
            )
        else:
            segment_stats = plot_data.groupby(dimension, observed=True)[metrics].apply(
                agg_func
            )
            ...
            for metric in metrics:
                ...
                if series.min() >= 0:
                    ...
                    segment_stats[metric] = (
                        0.5 if min_val == 0 else 1 - (series / min_val)
                    )
                else:
                    pos_mask = series >= 0
                    neg_mask = ~pos_mask
        ...
        polar_data = segment_stats.reset_index().melt(
            id_vars=[dimension], var_name="Metric", value_name="Value"
        )
        if labels:
        ...
        is_text_auto_false = text_auto is False
        fig.update_xaxes(
        ...
        # 8. Update layout
        dimension_title = (
            dimension if (not labels or dimension not in labels) else labels[dimension]
        )
        fig.update_layout(
            title_text=title or "Segment Analysis",
            title_y=0.96,
            width=width,
            height=height,
            showlegend=True,
    ...
    def segment_table(
        ...
        if isinstance(agg_func, str) and agg_func.startswith("p"):
            try:
                ...
                agg_df = df.groupby(dimension, observed=True)[metrics].quantile(
                    percentile
                )
            except ValueError:
        ...
        formatted_df["% of Total Count"] = formatted_df["% of Total Count"].apply(
            lambda x: f"{x:.2%}" if pd.notna(x) else na_rep
        )
        formatted_df[metrics] = formatted_df[metrics].map(
            lambda x: f"{x:.2f}" if pd.notna(x) else na_rep
        )
        ...
        styled_df = (
            formatted_df.style.set_caption(title)
        ...
        for dimension in dimensions:
            ...
            def gradient_columns(col):
                numeric_values = pd.to_numeric(
                    col.str.replace(na_rep, "NaN"), errors="coerce"
                )
                valid_vals = numeric_values[~numeric_values.isna()]
            ...
            # Apply styling
            styled_table = (
                formatted_df.style.set_caption(
    ...
    def metric_by_dimensions_plot(
        ...
        **plotly_kwargs,
        ...
        for dim in dimensions:
            ...
            if sort_bars:
                if color:
                    ...
                    sort_order = (
                        dim_data.groupby("Segment")[metric]
                        .mean()
                        .sort_values(ascending=False)
                        .index
                    )
                    dim_data["Segment"] = pd.Categorical(
                        dim_data["Segment"], categories=sort_order, ordered=True
                    )
        ...
        fig.update_yaxes(
            matches=None, showticklabels=True, categoryorder="total ascending"
        )
        fig.update_layout(
