...
class TestFrameOnAnomaly:
    ...
    def test_column_filtering(self, sample_data_with_anomalies):
        ...
        result1 = analyzer.anomalies_report(
            ...
            include_columns=['missing1', 'missing2']
        )
        ...
        result2 = analyzer.anomalies_report(
            anomaly_type='missing',
            exclude_columns=['text', 'value']
        )
        assert result2 is not None
        
    @pytest.mark.parametrize("anomaly_type", ['missing', 'duplicate', 'outlier', 'zero', 'negative'])
    def test_detect_simultaneous_anomalies(self, sample_data_with_anomalies, anomaly_type):
        """Test detection of simultaneous anomalies"""
        analyzer = FrameOnAnomaly(sample_data_with_anomalies)
        
        # Test with two anomaly types
        result = analyzer.detect_simultaneous_anomalies(
            columns=['missing1', 'missing2']
            , anomaly_type=anomaly_type
            , return_results=True
        )

    @pytest.mark.parametrize("anomaly_type", ['missing', 'duplicate', 'zero', 'negative'])
    def test_anomalies_by_categories_basic(self, sample_data_with_anomalies, anomaly_type):
        """Test anomalies_by_categories with basic anomaly types"""
        analyzer = FrameOnAnomaly(sample_data_with_anomalies)
        analyzer.anomalies_by_categories(anomaly_type=anomaly_type)

    @pytest.mark.parametrize("method", ['iqr', 'zscore', 'quantile'])
    def test_anomalies_by_categories_outlier_methods(self, sample_data_with_anomalies, method):
    ...
    @pytest.mark.parametrize("exact", [True, False])
    def test_anomalies_by_categories_duplicate_options(self, sample_data_with_anomalies, exact):
    ...
    def test_anomalies_by_categories_threshold_validation(self, sample_data_with_anomalies):
