...
class TestCohortAnalyzer:
    ...
    def test_basic_retention(self, sample_transactions):
        ...
        result = analyzer.cohort(
            user_id_col="user_id", date_col="date", mode="retention"
        )
    ...
    def test_input_validation(self, sample_transactions):
        ...
        with pytest.raises(ValueError):
            analyzer.cohort(user_id_col="invalid_column", date_col="date")
    ...
    def test_edge_cases(self):
        ...
        df = FrameOn({"user_id": ["user1"], "date": [datetime(2023, 1, 1)]})
        analyzer = CohortAnalyzer(df)
        with pytest.raises(ValueError):
            analyzer.cohort(user_id_col="user_id", date_col="date", mode="revenue")
    ...
    def test_different_granularities(self, sample_transactions):
        ...
        for granularity in ["day", "week", "month", "quarter"]:
            result = analyzer.cohort(
                user_id_col="user_id", date_col="date", granularity=granularity
            )
            assert result is not None
    ...
    @pytest.mark.parametrize(
        "summary_stat", ["mean", "weighted_mean", "median", "min", "max"]
    )
    ...
    def test_visualization_outputs(self, sample_transactions):
        ...
        heatmap = analyzer.cohort(
            user_id_col="user_id", date_col="date", display_mode="matrix"
        )
