"""
Cohort analysis functionality for tracking user behavior over time.

Provides tools for calculating and visualizing:
- Retention rates
- Revenue metrics (LTV, ARPU, etc.)
- Marketing performance (ROI, CAC ratio)
...
class CohortConfig:
    ...
    summary_stat: Optional[
        List[Literal["mean", "weighted_mean", "median", "min", "max"]]
    ] = "median"
...
class CohortAnalyzer:
    ...
    def cohort(
        ...
        try:
            ...
            if (
                self.config_cohort.marketing_costs_df is not None
                and "_cohort_" in self.config_cohort.marketing_costs_df.columns
            ):
                self.config_cohort.marketing_costs_df.drop(
                    columns="_cohort_", inplace=True, errors="ignore"
                )
            self.df.drop(
                columns=["_first_activity_", "_cohort_", "_lifetime_"],
                inplace=True,
                errors="ignore",
            )
            raise
    ...
    def _validate_parameters(self, params: Dict[str, Any]) -> None:
        ...
        if (
            "min_cohort_size" in params
            and "max_cohort_size" in params
            and params["min_cohort_size"] is not None
            and params["max_cohort_size"] is not None
            and params["min_cohort_size"] > params["max_cohort_size"]
        ):
    ...
    def _validate_data(self) -> None:
        ...
        if (
            self.config_cohort.marketing_costs_df is not None
            and self.config_cohort.mode in ["romi", "ltv_cac_ratio"]
        ):
    ...
    def _check_duplicate_user_dates(self) -> None:
        ...
        duplicates = self.df.duplicated(
            subset=[config.user_id_col, config.date_col], keep=False
        )
        if duplicates.any():
            pct = duplicates.mean() * 100
            print(
                f"Warning: Found {duplicates.sum()} duplicate user-date pairs ({pct:.2f}% of data)"
            )
    ...
    def _check_negative_revenue(self) -> None:
        ...
        if config.revenue_col and config.revenue_col in self.df.columns:
            ...
            if negative.any():
                ...
                print(
                    f"Warning: Found {negative.sum()} negative revenue values ({pct:.2f}% of data)"
                )
    ...
    def _validate_marketing_period_alignment(self) -> None:
        ...
        min_marketing_date = config.marketing_costs_df[
            config.marketing_costs_date_col
        ].min()
        max_marketing_date = config.marketing_costs_df[
            config.marketing_costs_date_col
        ].max()
        ...
        if min_marketing_date > min_main_date:
            print(
                f"Warning: Marketing data starts {min_marketing_date} while main data starts {min_main_date}"
            )
        if max_marketing_date < max_main_date:
            print(
                f"Warning: Marketing data ends {max_marketing_date} while main data ends {max_main_date}"
            )
    ...
    def _check_missing_periods(self) -> None:
        ...
        if config.marketing_costs_df is not None and config.mode in [
            "romi",
            "ltv_cac_ratio",
        ]:
    ...
    def _check_df_missing_periods(
        self, df: pd.DataFrame, date_col: str, data_name: str
    ) -> None:
        """
    ...
    def _validate_marketing_data(self) -> None:
        ...
        if self.config_cohort.marketing_costs_df is not None:
            ...
            if not pd.api.types.is_datetime64_any_dtype(
                df[self.config_cohort.marketing_costs_date_col]
            ):
                raise ValueError("Marketing costs date column must be datetime type")

            if not pd.api.types.is_numeric_dtype(
                df[self.config_cohort.marketing_costs_value_col]
            ):
    ...
    def _get_required_columns(self) -> List[str]:
        ...
        requirements = {
            ...
            mode, [(config.user_id_col, "user_id_col"), (config.date_col, "date_col")]
        )

        # Check for missing parameters
        missing_params = [
            param_name for value, param_name in requirements if value is None
        ]
        ...
        required_cols = [
            value
            for value, _ in requirements
            if value is not None
            and not isinstance(value, pd.DataFrame)  # Exclude DataFrame objects
        ]
        ...
        if config.revenue_col and not pd.api.types.is_numeric_dtype(
            self.df[config.revenue_col]
        ):
        ...
        if mode in ["romi", "ltv_cac_ratio"]:
            ...
            if (
                not config.marketing_costs_date_col
                or not config.marketing_costs_value_col
            ):
                raise ValueError(
                    "Marketing costs date and value columns must be specified"
                )
            if config.marketing_costs_date_col not in config.marketing_costs_df.columns:
                raise ValueError(
                    f"Missing required columns in DataFrame: {config.marketing_costs_date_col}"
                )
            if (
                config.marketing_costs_value_col
                not in config.marketing_costs_df.columns
            ):
                raise ValueError(
                    f"Missing required columns in DataFrame: {config.marketing_costs_value_col}"
                )
    ...
    def _calculate_cohorts(self) -> None:
        ...
        df["_first_activity_"] = df.groupby(config.user_id_col)[
            config.date_col
        ].transform("min")
        ...
        if config.granularity == "day":
            ...
            max_lifetime = (
                df[config.date_col].dt.to_period("D").max()
                - df[config.date_col].dt.to_period("D").min()
            )
            ...
            max_lifetime = (
                df[config.date_col].dt.to_period("W").max()
                - df[config.date_col].dt.to_period("W").min()
            )
            self.max_lifetime = max_lifetime.n
            ...
            if config.month_lifetime_method == "30days":
                ...
                df["_lifetime_"] = np.floor(
                    time_diff.dt.total_seconds() / (3600 * 24 * 30)
                ).astype(int)
                start_of_month = df[["_cohort_"]].drop_duplicates()
                all_cohorts = pd.period_range(
                    start=df[config.date_col].min(),
                    end=df[config.date_col].max(),
                    freq=freq_map[self.config_cohort.granularity],
                )
                start_of_month = (
                ...
                effective_current_month_day = np.minimum(
                    first_activity_day, activity_last_day_of_month
                )
                ...
                max_lifetime = (
                    df[config.date_col].dt.to_period("M").max()
                    - df[config.date_col].dt.to_period("M").min()
                )
                self.max_lifetime = max_lifetime.n
            else:
                raise ValueError(
                    f"Unsupported month_lifetime_method: {config.month_lifetime_method}. "
                    "Can be one of calendar or 30days"
                )
            ...
            max_lifetime = (
                df[config.date_col].dt.to_period("Q").max()
                - df[config.date_col].dt.to_period("Q").min()
            )
            self.max_lifetime = max_lifetime.n
            df["_lifetime_"] = df["_lifetime_"].clip(lower=0)
        else:
    ...
    def _calculate_marketing_period(self) -> None:
        ...
        if not pd.api.types.is_datetime64_any_dtype(
            df[config.marketing_costs_date_col]
        ):
            raise ValueError(
                "Date column in marketing costs data must be datetime type."
            )
    ...
    def _calculate_user_metrics(self) -> None:
        ...
        if config.mode == "retention":
            ...
            users = (
                df.groupby(["_cohort_", "_lifetime_"], observed=False)[
                    config.user_id_col
                ]
                .nunique()
    ...
    def _calculate_order_metrics(self) -> None:
        ...
        if config.mode in ["orders", "sales"]:
            ...
            orders = df.groupby(
                ["_cohort_", "_lifetime_", config.user_id_col], observed=False
            )[config.order_id_col].nunique()
            result = (
                orders.groupby(["_cohort_", "_lifetime_"], observed=False)
                .mean()
                .unstack(fill_value=0)
            )
            ...
            # Average order value
            revenue = df.groupby(["_cohort_", "_lifetime_"], observed=False)[
                config.revenue_col
            ].sum()
            orders = df.groupby(["_cohort_", "_lifetime_"], observed=False)[
                config.order_id_col
            ].nunique()
    ...
    def _calculate_revenue_metrics(self) -> None:
        ...
        if config.mode == "revenue":
            ...
            active_users = df.groupby(["_cohort_", "_lifetime_"], observed=False)[
                config.user_id_col
            ].nunique()
    ...
    def _calculate_marketing_metrics(self) -> None:
        ...
        ltv_cumulative = (
            self.df.groupby(["_cohort_", "_lifetime_"], observed=False)[
                config.revenue_col
            ]
            .sum()
        ...
        cac = (
            config.marketing_costs_df.groupby("_cohort_", observed=False)[
                config.marketing_costs_value_col
            ]
            .sum()
            .div(self.cohort_sizes)
        ...
        merged = pd.merge(ltv_cumulative, cac, on="_cohort_", how="left")
    ...
    def _postprocess_results(self) -> None:
        ...
        if config.granularity == "month" and config.month_lifetime_method == "30days":
            ...
            result = result.reindex(
                index=all_cohorts, columns=all_periods, fill_value=0
            )
            self.cohort_sizes = self.cohort_sizes.reindex(
                index=all_cohorts, fill_value=0
            )
    ...
    def _filter(self) -> None:
        ...
        if config.min_cohort_size or config.max_cohort_size:
            ...
            if not excluded_cohorts.empty:
                ...
                print(
                    f"Excluded {len(excluded_cohorts)} cohorts ({pct:.1f}%) outside {', '.join(size_range)} size range"
                )
        ...
        if not config.include_period0 and 0 in result.columns:
            ...
            result = result.iloc[:-1]
            if (
                self.config_cohort.summary_stat
                and not self.summary_stat_result.empty
                and 0 in self.summary_stat_result.columns
            ):
    ...
    def _calculate_summary_stat(self) -> None:
        ...
        if stat == "mean":
            ...
            cohort_sizes_extend = pd.DataFrame(
            ...
            weighted_mean = (
                result.mul(cohort_sizes_extend[0], axis=0).sum().astype(float)
            )
    ...
    def _plot_heatmap(self) -> CustomFigure:
        ...
        if config.mode == "retention":
            hovertemplate = (
                "Cohort: %{y}<br>Lifetime: %{x}<br>Value: %{z:.2%}<extra></extra>"
            )
        else:
            hovertemplate = (
                "Cohort: %{y}<br>Lifetime: %{x}<br>Value: %{z:"
                + text_auto
                + "}<extra></extra>"
            )
        fig_main.update_traces(hovertemplate=hovertemplate)
        ...
        if summary_stat and not self.summary_stat_result.empty:
            ...
            fig_mini.update_traces(
                hovertemplate="Cohort: %{y}<br>Lifetime: %{x}<br>Value: %{z:"
                + text_auto
                + "}<extra></extra>"
            )
        ...
        xaxis_title_text = (
            xaxis_title_text
            if xaxis_title_text
            else f"Lifetime ({self.config_cohort.granularity})"
        )
        fig.update_xaxes(
        ...
        fig.update_layout(
            coloraxis=dict(colorscale=color_continuous_scale, showscale=show_colorbar),
            title_text=title,
            height=height,
            width=width,
    ...
    def _plot_summary(self) -> CustomFigure:
        ...
        fig = px.line(
            ...
            y="value",
            labels={
                "_lifetime_": f"Lifetime ({self.config_cohort.granularity})",
                "value": yaxis_label,
            },
        ...
        # Customize layout
        fig.update_layout(
            height=height, width=width, margin=dict(l=10, r=10, b=10, t=50)
        )
        if config.mode == "retention":
            fig.update_layout(
                yaxis=dict(
    ...
    def _determine_decimal_places(self, data: pd.DataFrame) -> int:
