"""
Heatmap visualization utilities with configurable preprocessing.

This module provides tools for creating heatmaps with support for pivot tables,
data filtering, and flexible configuration options. Includes specialized
preprocessing for categorical and numerical data visualization.
"""

from dataclasses import dataclass, fields
from typing import Any, Dict, Literal, Optional, Union, get_type_hints
...
class HeatmapBuilder:
    ...
    def build(self, **kwargs) -> CustomFigure:
        ...
        if pd.api.types.is_datetime64_any_dtype(
            self.plotly_kwargs["data_frame"].columns
        ):
            self.plotly_kwargs["data_frame"].columns = self.plotly_kwargs[
                "data_frame"
            ].columns.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def _validate_params(self) -> None:
        ...
        if fill_value and (
            not isinstance(fill_value, int) or not isinstance(fill_value, float)
        ):
        ...
        trim_fields = ["x", "y"]
        top_n_params = [
            getattr(config, f"trim_top_n_{field}")
            for field in trim_fields
            if getattr(config, f"trim_top_n_{field}")
        ]
        ...
        if do_pivot:
            ...
            if not all(
                [is_x_string or is_x_grouper, is_y_string or is_y_grouper, is_z_string]
            ):
            ...
            if agg_func not in [
                "count",
                "nunique",
            ] and not pd.api.types.is_numeric_dtype(data_frame[z]):
                raise ValueError(
                    "z must be numeric dtype, not numeric allow for agg_func = count/nunique"
                )
        ...
        if (
            any(getattr(config, p) for p in top_n_params)
            and not config.trim_top_n_agg_func
        ):
            raise ValueError(
                "trim_top_n_agg_func must be specified when using top_n_trim parameters"
            )
    ...
    def _apply_trimming_categories(self) -> None:
        ...
        if self.config.do_pivot and (df.shape[0] < 2 or df.shape[1] < 2):
            raise ValueError(
                "Resulting DataFrame after filtering is too small (min 2x2 required)"
            )
    ...
    def _prepare_data(self) -> None:
        ...
        if self.config.hide_first_column:
            self.plotly_kwargs["data_frame"] = self.plotly_kwargs["data_frame"].iloc[
                :, 1:
            ]
    ...
    def _process_category_orders(self) -> None:
        ...
        def _process_axis(axis: str, axis_col: str):
            ...
            if isinstance(order_spec, list):
                ...
                if "category" in order_spec:
                    ...
                    sorted_values = agg_values.sort_values(
                        ascending=ascending
                    ).index.tolist()
        ...
        if self.config.x:
            ...
            raise ValueError(
                "For sorting by category_orders without do_pivot df.column.name must be defined"
            )
        ...
        if self.config.y:
            ...
            raise ValueError(
                "For sorting by category_orders without do_pivot df.index.name must be defined"
            )
    ...
    def _process_labels(self) -> None:
        ...
        new_labels = {}
        label_map = {self.config.x: "x", self.config.y: "y", self.config.z: "color"}
        for label in labels:
    ...
    def _create_figure(self) -> None:
        ...
        self.figure = px.imshow(
            img=self.plotly_kwargs.pop("data_frame"), **self.plotly_kwargs
        )
