...
class TestSeriesOnPreproc:
    ...
    def test_check_group_counts(self, df_with_time_column):
        ...
        result = df_with_time_column['missing1'].preproc.check_group_counts(category_columns=['text'], return_report=True)
    ...
    def test_edge_cases(self):
        ...
        result = df['value'].preproc.impute_missing(strategy='constant', imputer_params={'fill_value': 0})
        assert (result == 0).all()
        
    # Parametrized tests for method selection parameters
    @pytest.mark.parametrize("method", ['equal_intervals', 'quantiles', 'custom_bins', 'clustering', 'rules'])
    def test_to_categorical_methods(self, numeric_series, method):
        ...
        if method == 'custom_bins':
            ...
            result = preproc.to_categorical(method=method, rules={"High": lambda x: x > 1, "Low": lambda x: x < -1, "Mid": "default"})
        else:
    ...
    @pytest.mark.parametrize("method", ['log', 'boxcox', 'yeojohnson', 'sqrt', 'reciprocal', 'zscore', 'robust', 'quantile'])
    ...
    @pytest.mark.parametrize("case_format", ['title', 'lower', 'upper', 'sentence', 'none'])
    ...
    def test_fill_missing_by_category_with_missing_categories(self, df_with_time_column):
