...
class TestTextAnalyzer:
    ...
    def test_sentiment_analysis(self, sample_text_data):
        ...
        # Test vader method
        vader_result = analyzer.sentiment(text_column="text", method="vader")
        assert vader_result is not None

        # Test textblob method
        textblob_result = analyzer.sentiment(text_column="text", method="textblob")
    ...
    def test_word_frequency(self, sample_text_data):
        ...
        # Test top words
        top_result = analyzer.word_frequency(text_column="text", show="top")
        assert top_result is not None

        # Test bottom words
        bottom_result = analyzer.word_frequency(text_column="text", show="bottom")
    ...
    def test_input_validation(self, sample_text_data):
        ...
        with pytest.raises(ValueError):
            analyzer.sentiment(text_column="text", method="invalid_method")

        # Test invalid show parameter
        with pytest.raises(ValueError):
            analyzer.word_frequency(text_column="text", show="invalid")
    ...
    def test_edge_cases(self):
        ...
        with pytest.raises(ValueError):
            analyzer.sentiment(text_column="text")

        # Test empty text
        df = FrameOn({"text": ["", "   ", None], "category": ["a", "b", "c"]})
        analyzer = TextAnalyzer(df)
        result = analyzer.sentiment(text_column="text")
