"""
Distribution visualization utilities for boxplots and violin plots.

This module provides tools for creating boxplots and violin plots with
enhanced features including time series support, categorical data handling,
and dual plot configurations. Supports both basic and advanced distribution
visualization needs.
"""
...
class DistributionPlotBuilder:
    ...
    def _prepare_plotly_kwargs(self) -> None:
        ...
        if data is not None:
            ...
            if not data_dict:
                raise ValueError(
                    "Either 'data' or at least one of 'x', 'y', 'color', "
                    "'facet_row', 'facet_col', 'animation_frame' must be provided."
                )
    ...
    def _validate_params(self) -> None:
        ...
        if show_dual:
            if upper_quantile == 1 and lower_quantile == 0:
                raise ValueError(
                    "For show_dual mode, lower_quantile or upper_quantile must be defined"
                )

            conflicting_options = [
                opt
                for opt in ["animation_frame", "facet_col", "facet_row"]
    ...
    def _determine_cat_num_columns(self) -> None:
        ...
        if x_col and y_col:
            ...
            if not is_x_cat and not is_y_cat:
                raise ValueError(
                    "For base mode both x and y cannot be numeric - one must be categorical."
                )
            ...
            if isinstance(order_spec, list):
                ...
                if order_spec not in valid_methods:
                    raise ValueError(
                        ...
                        "Valid methods: {sorted(valid_methods)}"
                    )
    ...
    def _trim_data(self) -> None:
        ...
        if group_cols:
            ...
            mask = df.groupby(group_cols, observed=False)[num_column].transform(
                trim_group
            )
    ...
    def _create_dual_plot(self, fig_left: go.Figure, fig_right: go.Figure) -> go.Figure:
        ...
        fig = make_subplots(
            rows=1, cols=2, shared_yaxes=shared_yaxes, horizontal_spacing=0.05
        )
        ...
        fig.update_yaxes(
            title_text=fig_left.layout["yaxis"]["title"]["text"], row=1, col=1
        )
        # Update layout
        fig.update_layout(
    ...
    def _update_layout(self) -> None:
        ...
        if not self.plotly_kwargs.get("width"):
            layout_updates["width"] = (
                800 if mode == "time_series" else (900 if show_dual else 700)
            )
        if not self.plotly_kwargs.get("height"):
            layout_updates["height"] = 400

        if color:
            layout_updates.update({"legend_position": "top", "legend_y": 1.08})
            if not self.config.show_legend_title:
                legend_title_text = ""
            else:
                legend_title_text = self.plotly_kwargs.get("labels", {}).get(
                    color, color
                )
            layout_updates.update({"legend_title_text": legend_title_text})
