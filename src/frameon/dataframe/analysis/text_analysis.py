"""
Text analysis tools for processing and visualizing textual data.
...
class TextAnalyzer:
    ...
    def sentiment(
        ...
        if not is_text_column(df[text_column]):
            raise ValueError("text_column must contain text")
        ...
        if clean_text:
            df[text_column] = (
                df[text_column].str.replace(r"[^\w\s]", "", regex=True).str.lower()
            )
        ...
        if show_stats:
            ...
            sentiment_counts_display["Percentage"] = (
                sentiment_counts_display["Percentage"].astype(str) + "%"
            )
            sentiment_counts_display["Count (Percentage)"] = (
                sentiment_counts_display["Count"].astype(str)
                + " ("
                + sentiment_counts_display["Percentage"]
                + ")"
            )
            sentiment_counts_display = sentiment_counts_display[
            ...
            res_df = (
                pd.concat([statistics, sentiment_counts_display], axis=1)
        ...
        if show_distribution:
            ...
            fig.add_trace(
                ...
                .update_traces(
                    hovertemplate="Score: %{x}<br>Probability: %{y}<extra></extra>"
                )
                .data[0],
                row=2,
                col=1,
            ...
            fig.add_trace(
                ...
                .update_traces(hovertemplate="Score: %{x}<extra></extra>")
                .data[0],
                row=1,
                col=1,
            )
            ...
            fig.update_yaxes(title_text="Probability", row=2, col=1, domain=[0, 0.92])

            # Update x-axis for box
            fig.update_xaxes(visible=False, row=1, col=1)

            # Update y-axis for box
            fig.update_yaxes(visible=False, row=1, col=1, domain=[0.93, 1])
    ...
    def word_frequency(
        ...
        **wordcloud_kwargs: Any,
        ...
        horizontal_spacing : float
            For show = 'both'. Space between subplot columns in normalized plot coordinates.
        ...
        if show == "both":
            ...
            top_words.reverse()
            bottom_words = (
                sorted_word_freq[-n:] if len(sorted_word_freq) > n else sorted_word_freq
            )
            # Create DataFrames
            ...
            fig.add_trace(
                px.bar(x=top_df["freq"], y=top_df["word"], text_auto=text_auto).data[0],
                row=1,
                col=1,
            )
            ...
            fig.add_trace(
                px.bar(
                    x=bottom_df["freq"], y=bottom_df["word"], text_auto=text_auto
                ).data[0],
                row=1,
                col=2,
            )
            ...
            fig.update_layout(
                ...
                showlegend=False,
                height=(
                    height if height else max(400, n * 20)
                ),  # Dynamic height based on n
                width=width if width else 900,
                margin=dict(l=50, r=50, b=50, t=80),
            )
            ...
            if show == "top":
                ...
                words = (
                    sorted_word_freq[-n:]
                    if len(sorted_word_freq) > n
                    else sorted_word_freq
                )
                default_title = f"Bottom {n} Least Frequent Words"
            else:
                raise ValueError(
                    f"Invalid show parameter must be one of {valid_show_parameter}"
                )
