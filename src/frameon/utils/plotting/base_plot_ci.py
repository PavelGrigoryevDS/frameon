"""
Confidence interval visualization utilities.

This module provides tools for creating plots with statistical confidence intervals
calculated using t-statistics. Supports grouped data and multiple orientation options.
"""
...
def create_plot_ci(
    ...
    if orientation not in ["v", "h"]:
        raise ValueError(
            "orientation must be either 'v' (vertical) or 'h' (horizontal)"
        )

    if legend_position not in ["top", "right", "bottom", "left"]:
        raise ValueError(
            "legend_position must be one of: 'top', 'right', 'bottom', 'left'"
        )
    ...
    if df[group_cols].isna().any().any():
        warnings.warn(
            "Grouping columns contain NaN values which will be excluded",
            UserWarning,
        )
        df = df.dropna(subset=group_cols)

    summary_df = (
        df.groupby(group_cols, observed=False)[num_col]
        .agg(["mean", "std", "count"])
        .reset_index()
    )
    ...
    if color:
        ...
        sub_offsets = np.linspace(
            -group_spacing / 2, group_spacing / 2, len(sub_categories)
        )
        sub_offset_map = {
            cat: offset for cat, offset in zip(sub_categories, sub_offsets)
        }
    ...
    hover_data = {cat_col: True, "mean": ":.2f", "ci": ":.2f"}
    if color:
    ...
    if show_annotations:
        ...
        for i, row in summary_df.iterrows():
            text = (
                f"{row['mean']:{annotation_format}} ± {row['ci']:{annotation_format}}"
            )
            if orientation == "v":
                annotations.append(
                    dict(
