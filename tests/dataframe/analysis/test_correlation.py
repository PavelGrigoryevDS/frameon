...
class TestCorrelationAnalyzer:
    ...
    def test_different_methods(self, sample_numeric_data):
    ...
    def test_significance_level(self, sample_numeric_data):
        """Test significance level parameter"""
        analyzer = CorrelationAnalyzer(sample_numeric_data)
        for level in [0.01, 0.05, 0.1]:
            result = analyzer.corr_matrix(significance_level=level)
            assert result is not None
