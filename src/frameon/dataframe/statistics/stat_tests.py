...
This module provides tools for performing various statistical tests,
including hypothesis testing, regression analysis, and feature importance evaluation.
"""
...
class StatisticalTests:
    ...
    def normality(
        ...
        if non_normal_groups:
            rprint(
                "[bright_red]Warning: Non-normal distributions detected in groups: "
                f"{', '.join(non_normal_groups)}[/bright_red]"
            )
        else:
        ...
        if show_qqplot and len(groups) > 0:
            ...
            fig, axes = plt.subplots(
                n_rows, n_cols, figsize=(width, height), squeeze=False
            )
    ...
    def levene(
        self, dv: str, between: str, alpha: float = 0.05, return_results: bool = False
        ...
        result = pg.homoscedasticity(
            data=df, dv=dv, group=between, method="levene", alpha=self.config.alpha
        )
        ...
        conclusion = (
            f"At alpha = {self.config.alpha}, we "
            f"{'[bright_red]reject[/bright_red]' if p_value < self.config.alpha else '[green]fail to reject[/green]'} "
    ...
    def _validate_dv_and_between(self, test_name: str) -> None:
        ...
        for factor in between_factors:
            if factor not in df.columns:
                raise ValueError(
                    f"Between factor column '{factor}' not found in DataFrame"
                )
            ...
            if test_name in ["ttest", "mannwhitney"]:
                ...
                if self.config.reference_group:
                    ...
                    if (
                        self.config.alternative != "two-sided"
                        and self.config.reference_group != groups[0]
                    ):
                        groups = groups[::-1]
                elif self.config.alternative != "two-sided":
                    raise ValueError(
                        "For one-tailed tests, reference_group must be specified"
                    )
    ...
    def _check_and_handle_na(self, dv: str, between: Union[str, List[str]]) -> Tuple:
        ...
        if between_factors:
            ...
            for factor in between_factors:
                if (
                    df[factor].nunique() <= 20
                ):  # Only show if reasonable number of groups
                    na_by_group = (
                        df.groupby(factor, observed=True)[dv]
    ...
    def ttest(
        ...
        h1 = (
            ...
            or {
                ...
                "greater": f"H1: The mean of {reference_group or group1} is greater than "
                f"{group2 if reference_group else group2}",
                "less": f"H1: The mean of {reference_group or group1} is less than "
                f"{group2 if reference_group else group2}",
            }[alternative]
        )
        text = f"""
        ...
        desc_stats[["mean", "std", "min", "max"]] = (
            desc_stats[["mean", "std", "min", "max"]].round(2).astype(str)
        )
        ...
        levene_result = pg.homoscedasticity(
            data=df, dv=dv, group=between, method="levene", alpha=alpha
        )
        ...
        if correction is None:
            ...
            if correction is False:
                text = "- Using Student's t-test - \n\n"
            elif correction is True:
                text = "- Using Welch's t-test - \n\n"
    ...
    def mwu(
        ...
        h0 = (
            self.config.h0
            or f"H0: The distribution of {group1} equals the distribution of {group2}"
        )
        h1 = (
            self.config.h1
            or {
                "two-sided": f"H1: The distribution of {group1} differs from the distribution of {group2}",
                "greater": f"H1: The distribution of {reference_group or group1} is stochastically greater than "
                f"{group2 if reference_group else group2}",
                "less": f"H1: The distribution of {reference_group or group1} is stochastically less than "
                f"{group2 if reference_group else group2}",
            }[alternative]
        )
        ...
        desc_stats[["median", "mean", "std", "min", "max"]] = (
            desc_stats[["median", "mean", "std", "min", "max"]].round(2).astype(str)
        )
        if any(desc_stats["count"] < 30):
            text = (
                "[bold]🔹 Descriptive Statistics[/bold]\n\n"
        ...
        rbc_effect = (
            "small" if abs(rbc) < 0.3 else "medium" if abs(rbc) < 0.5 else "large"
        )
        cles_interpretation = (
            "small"
            if abs(cles - 0.5) < 0.1
            else "medium" if abs(cles - 0.5) < 0.2 else "large"
        )
    ...
    def anova(
        ...
        # Prepare report header
        design_type = (
            "One-way" if is_oneway else f"{len(between_factors)}-way Factorial"
        )
        text = f"""
        ...
        if is_oneway:
            ...
            groups_str = ", ".join(groups[:3]) + (
                f", ... ({len(groups)} total)" if len(groups) > 3 else ""
            )
            ...
            h1 = (
                h1
                or f"H1: Significant main effects or interactions for factors: {factors_str}"
            )
        text = f"""
        [bold]🔹 Hypothesis Formulation[/bold]\n
        - {h0}
        ...
        if is_oneway:
            ...
            desc_stats[["mean", "std", "min", "max", "median"]] = desc_stats[
            ...
            if any(desc_stats["count"] < 30):
                ...
                text_output += (
                    "\n\n⚠️ [yellow]Warning: Small sample size (<30) in groups: "
                )
                text_output += f"{', '.join(map(str, small_groups))}[/yellow]"
            rprint(text_output)
            text_output = ""
        ...
        if is_oneway:
            levene_result = pg.homoscedasticity(
                data=df, dv=dv, group=between_factors[0], method="levene", alpha=alpha
            )
            rprint(text_output)
            text_output = ""
            display(levene_result.round(3).astype(str))
            ...
            equal_var = True
            text_output += (
                "\n\n- In a multifactorial ANOVA, homoscedasticity (equal variances) "
            )
            text_output += "is assumed without explicit testing.\n\n"
        # Perform appropriate ANOVA
        text_output += (
            f"[bold]🔹 {'Welch' if not equal_var else ''} ANOVA Results[/bold]"
        )
        if equal_var:
            aov = pg.anova(
                data=df,
            ...
            if not is_oneway:
                raise ValueError(
                    "Welch's ANOVA only available for one-way design. Consider data transformation."
                )
            aov = pg.welch_anova(data=df, dv=dv, between=between_factors[0])
        rprint(text_output)
        text_output = ""
        ...
        if is_oneway and equal_var:  # Power calculation for one-way normal ANOVA
            ...
            achieved_power = pg.power_anova(eta_squared=eta_sq, k=k, n=n, alpha=alpha)
        ...
        if not sig_effects.empty:
            text_output += "[bold]🔹 Post-hoc Analysis[/bold]"
            rprint(text_output)
            text_output = ""
            if is_oneway:
                # One-way post-hoc
                if equal_var:
                    ph = pg.pairwise_tukey(data=df, dv=dv, between=between_factors[0])
                    caption = "Tukey HSD for equal variances"
                else:
                    ph = pg.pairwise_gameshowell(
                        data=df, dv=dv, between=between_factors[0]
                    )
                    caption = "Games-Howell for unequal variances"
                ph = ph.round(3).astype(str)
                display(
                    style_dataframe(
                        ph, hide_columns=False, caption=caption, caption_font_size=14
                    )
                )

                text_output = (
                    "[bold]🔹 Conclusion:[/bold] [bright_red]Significant effects found "
                )
                text_output += "(p-unc = {aov['p-unc'].iloc[0]:.3f})[/bright_red]\n"
            else:
            ...
            if is_oneway:
                rprint(
                    "[bold]🔹 Conclusion:[/bold] [green]No significant effects found[/green]\n"
                )
            else:
                rprint(
                    "[bold]🔹 Conclusion:[/bold] [green]No significant main effects or interactions found[/green]"
                )
    ...
    def kruskal(
        ...
        desc_stats[["median", "mean", "std", "min", "max"]] = desc_stats[
        ...
        kw_result = pg.kruskal(data=df, dv=dv, between=between)
        ...
        if kw_punc < alpha:
            ...
            dunn_results = posthoc_dunn(
                df, val_col=dv, group_col=between, p_adjust="holm"
            )
            ...
            text = (
                "[bold]🔹 Conclusion:[/bold] [bright_red]Significant difference found "
            )
            text += f"(p-unc = {kw_punc:.3f})[/bright_red]"
    ...
    def chi2_independence(self, x, y, test="pearson", alpha=0.05) -> None:
        ...
        if test not in [
            ...
            raise ValueError(
                f"Invalid test type: {test}. Choose from 'pearson', 'cressie-read', "
                "'log-likelihood', 'freeman-tukey', 'mod-log-likelihood', 'neyman', or 'all'."
            )
        ...
        for col in [x, y]:
            ...
            if na_count > 0:
                ...
                na_info.append(
                    f"⚠️ Found {na_count} ({na_pct:.1f}%) missing values in '{col}'"
                )
        output_text = ""
        if na_info:
            output_text += "\n".join(na_info)
        ...
        output_text += "[bold]Categorical Association Report[/bold]\n\n"
        output_text += f"- {x} vs {y}\n"
        output_text += f"- Total observations: {n}\n\n"

        groups_x = df[x].unique()
        groups_y = df[y].unique()
        groups_str_x = ", ".join(map(str, groups_x[:3])) + (
            f", ... ({len(groups_x)} total)" if len(groups_x) > 3 else ""
        )
        groups_str_y = ", ".join(map(str, groups_y[:3])) + (
            f", ... ({len(groups_y)} total)" if len(groups_y) > 3 else ""
        )
        ...
        p_value_str = (
            f"[bright_red]{p_value:.3f}[/bright_red]"
            if p_value < alpha
            else f"[green]{p_value:.3f}[/green]"
        )
    ...
    def bootstrap(
        ...
        **kwargs,
        ...
        if method == "BCa" and n_resamples < 1000 and not parallel:
            warnings.warn(
                "BCa method recommended to use at least 1000 resamples for stable results"
            )
        # Validate inputs
        if not isinstance(df, pd.DataFrame):
        ...
        if not isinstance(dv, str) or dv not in df.columns:
            raise ValueError("dv must be a single column name present in DataFrame")

        if between is not None:
            if not isinstance(between, str) or between not in df.columns:
                raise ValueError("between must be a column name present in DataFrame")

            groups = self.df[between].unique()
            if len(groups) != 2:
                raise ValueError(
                    f"between must have exactly 2 unique groups, found {len(groups)}"
                )

            if reference_group is None:
                raise ValueError(
                    "reference_group parameter required when between is specified - "
                    "define which group should be first in comparison (e.g., reference_group='control')"
                )
            elif reference_group not in groups:
                raise ValueError(
                    f"reference_group group '{reference_group}' not found in {between} column"
                )

        if not isinstance(statistic, (str, Callable)):
            raise TypeError(
                "statistic must be either string or callable. "
        ...
        if parallel:
            ...
            if method == "BCa" and actual_resamples < 1000:
                warnings.warn(
                    f"BCa method using {actual_resamples} resamples (recommended >= 1000)"
                )
            ...
            with tqdm_joblib(tqdm(desc="Bootstrapping", total=n_jobs)):
                bootstrap_dists = Parallel(n_jobs=n_jobs)(
                    delayed(parallel_bootstrap_task)(i) for i in range(n_jobs)
                )
            ...
            if method == "percentile":
                ...
                z0 = norm.ppf(
                    np.clip(prop_less, 1e-12, 1 - 1e-12)
                ...
                if between is None:
                    ...
                    for i in range(n1):
                        jack_stats.append(
                            statistic_func(np.delete(data[0], i), data[1])
                        )

                    # Leave-one-out for second sample
                    for i in range(n2):
                        jack_stats.append(
                            statistic_func(data[0], np.delete(data[1], i))
                        )
                ...
                def get_bca_quantile(alpha):
                    ...
                    if np.abs(denominator) < 1e-8 or not np.isfinite(denominator):
                        warnings.warn(
                            "BCa adjustment unstable, falling back to percentile"
                        )
                        return alpha
            ...
            if method == "BCa" and n_resamples < 1000:
                warnings.warn(
                    f"BCa method using {n_resamples} resamples (recommended >= 1000)"
                )
        ...
        if not return_results:
            ...
            return BootstrapResult(
                table=result_table, bootstrap_distribution=bootstrap_dist, plot=fig
            )

    def _plot_bootstrap_distribution(
        self, bootstrap_dist, ci_low, ci_high, title="Bootstrap Distribution"
    ) -> go.Figure:
    ...
    def glm(
        ...
        **kwargs: Any,
        ...
        missing : str
            How to handle missing values (default: 'drop')
        p_adjust : str
            Method used for testing and adjustment of p-values. Can be either the full name or initial letters.
    ...
    def ordered_model(
        ...
        **kwargs,
        ...
        if not pd.api.types.is_categorical_dtype(
            self.df[formula.split("~")[0].strip()]
        ):
            raise TypeError(
                "Dependent variable must be ordered categorical. Use pd.Categorical"
            )
        # Prepare data
        ...
        results = model.fit(method=method, **kwargs)

        # Apply p-value adjustment if requested
        if p_adjust and hasattr(results, "pvalues"):
            pvals = results.pvalues[1:]  # Exclude threshold parameters
            ...
            summary_df = pd.DataFrame(
                {
                    ...
                    "p_adj": np.concatenate(
                        [
                            [np.nan]
                            * (
                                len(results.params) - len(pvals_corrected)
                            ),  # For thresholds
                            pvals_corrected,
                        ]
                    ),
    ...
    def _check_multicollinearity(self, X: pd.DataFrame) -> None:
        ...
        vif_df = pd.DataFrame({"Variable": X.columns, "VIF": vif}).sort_values(
            "VIF", ascending=False
        )

        display(
            style_dataframe(
                vif_df.round(3).astype(str),
                hide_columns=False,
        ...
        if (vif_df["VIF"] > 10).any():
            ...
            rprint(
                "[yellow]Warning: VIF > 5 suggests moderate multicollinearity[/yellow]"
            )
    ...
    def _show_minimal_diagnostic_plots(
        ...
        try:
            ...
            rprint(
                "[yellow]⚠️ matplotlib/seaborn not available - plots skipped[/yellow]"
            )
    ...
    def feature_importance_analysis(
        ...
        if removed_rows > 0:
            removal_pct = (removed_rows / initial_rows) * 100
            rprint(
                f"[yellow]⚠️ Removed {removed_rows} rows ({removal_pct:.2f}%) with missing values[/yellow]"
            )
        ...
        if problem_type == "classification":
            ...
            if n_classes == 2:  # Binary classification
                ...
                metric_func = calculate_log_loss
        ...
        if problem_type == "classification" and n_classes > 2:
            ...
            if not all(hasattr(m, "predict_proba") for m in test_models):
                warnings.warn(
                    "Some models don't support predict_proba, falling back to accuracy"
                )
        ...
        if use_statistical:
            try:
                ...
                results["statistical_f_test"] = FeatureImportanceResult(
                    importance_df=stats_importance, metric_name="F-statistic"
                )
            except Exception as e:
                warnings.warn(
                    f"Statistical feature selection (F-test) failed: {str(e)}"
                )
        ...
        if use_l1:
            try:
                ...
                l1_importance = pd.DataFrame(
                    {"Feature": feature_columns, "Importance": np.abs(coef)}
                )
    ...
    def _plot_feature_importance(
        ...
        combined_importance = pd.merge(
            rf_importance_df, gb_importance_df, on="Feature", suffixes=("_rf", "_gb")
        )

        # Sort by combined importance (sum of RF and GB)
        combined_importance["Combined Importance"] = (
            combined_importance["Importance_rf"] + combined_importance["Importance_gb"]
        )
        combined_importance = combined_importance.sort_values(
            by="Combined Importance", ascending=False
        ).head(top_n_features)
        ...
        traces.append(
            go.Bar(
                x=combined_importance_df[
                    combined_importance_df["Model"] == "Importance_rf"
                ]["Importance"],
                y=combined_importance_df[
                    combined_importance_df["Model"] == "Importance_rf"
                ]["Feature"],
                name=rf_name,
                orientation="h",
                ...
                text=(
                    [
                        f"{x:.2f}"
                        for x in combined_importance_df[
                            combined_importance_df["Model"] == "Importance_rf"
                        ]["Importance"]
                    ]
                    if show_values
        ...
        traces.append(
            go.Bar(
                x=combined_importance_df[
                    combined_importance_df["Model"] == "Importance_gb"
                ]["Importance"],
                y=combined_importance_df[
                    combined_importance_df["Model"] == "Importance_gb"
                ]["Feature"],
        ...
        if use_statistical and "statistical_f_test" in results:
            ...
            traces.append(
                go.Bar(
                    ...
                    hovertemplate="<b>%{y}</b><br>Importance: %{x:.2f}<extra></extra>",
                    text=(
                        [f"{x:.2f}" for x in stats_top["Importance"]]
                        if show_values
                        else None
                    ),
                )
        ...
        if use_mutual_info and "mutual_information" in results:
            mutual_info_top = results["mutual_information"].importance_df.head(
                top_n_features
            )
            traces.append(
                go.Bar(
                    ...
                    hovertemplate="<b>%{y}</b><br>Importance: %{x:.2f}<extra></extra>",
                    text=(
                        [f"{x:.2f}" for x in mutual_info_top["Importance"]]
                        if show_values
                        else None
                    ),
                )
        ...
        if use_l1 and "l1_regularization" in results:
            ...
            traces.append(
                go.Bar(
                    ...
                    hovertemplate="<b>%{y}</b><br>Importance: %{x:.2f}<extra></extra>",
                    text=(
                        [f"{x:.2f}" for x in l1_top["Importance"]]
                        if show_values
                        else None
                    ),
                )
    ...
    def _adjust_regression_pvalues(self, results, p_adjust: str):
        ...
        results_df["p-corr"] = np.concatenate([[np.nan], pvals_corrected])  # Intercept
