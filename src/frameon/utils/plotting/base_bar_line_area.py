"""
Base plotting utilities for bar, line and area charts.

This module provides enhanced plotting functionality for basic chart types
with built-in data preprocessing and transformation capabilities.
Supports bar, line and area plots with consistent styling and configuration.
"""
...
FIGURE_CREATORS = {"bar": px.bar, "line": px.line, "area": px.area}
...
class BarLineAreaBuilder:
    ...
    def __init__(self, plot_type: Literal["bar", "line", "area"]):
        if plot_type not in FIGURE_CREATORS:
            raise ValueError(
                f"Invalid plot_type. Must be one of: {list(FIGURE_CREATORS.keys())}"
            )
    ...
    def build(self, **kwargs) -> CustomFigure:
        ...
        if (
            any(getattr(self.config, f"trim_top_n_{field}") for field in trim_fields)
            or self.config.show_top_and_bottom_n
        ):
            ...
            if key in config_fields:
                ...
                if (
                    ...
                    if not isinstance(value, expected_type.__args__):
                        raise TypeError(
                            f"Invalid type for '{key}'. Expected one of {expected_type.__args__}, got {type(value)}"
                        )
    ...
    def _prepare_plotly_kwargs(self) -> None:
        ...
        if data is not None:
            if isinstance(data, pd.DataFrame):
                ...
                raise ValueError(
                    "Unsupported type for 'data'. Must be one of DataFrame, Series, array-like, or dict."
                )
        else:
                ...
                if key in [
                    ...
                    if value is not None:
                        if isinstance(value, str):
                            raise ValueError(
                                f"Unsupported type for '{key}'. If 'data' is not provided, "
                                "'{key}' must be Series, array-like, or list."
                            )
                            ...
                            if value.ndim != 1:
                                raise ValueError(
                                    f"Only 1-dimensional arrays are supported for {key}."
                                )
                            data_dict[key] = value
                            plotly_kwargs[key] = key
                            ...
                            if not all(not isinstance(i, list) for i in value):
                                raise ValueError(
                                    f"Only 1-dimensional lists are supported for {key}."
                                )
                            data_dict[key] = value
                            plotly_kwargs[key] = key
                        else:
                            raise ValueError(
                                f"Unsupported type for '{key}'. Must be Series, array-like, or list."
                            )

            if not data_dict:
                raise ValueError(
                    "Either 'data' or at least one of 'x', 'y', 'color', 'facet_row', 'facet_col', "
                    "'animation_frame' must be provided."
                )
            ...
            if len(unique_lengths) != 1:
                raise ValueError(
                    f"All provided data must have the same length. Provided lengths: {lengths}"
                )
            ...
            if key in ["x", "y", "color", "facet_row", "facet_col", "animation_frame"]:
                if value is not None and isinstance(value, str):
                    if value not in df.columns:
                        raise ValueError(
                            f"Column '{value}' not found in the DataFrame. Available columns: {list(df.columns)}"
                        )
    ...
    def _validate_show_params(self) -> None:
        ...
        if sum([show_box, show_count, is_show_top_and_bottom_n]) > 1:
            raise ValueError(
                "show_box/show_count/show_top_and_bottom_n cannot be used together"
            )
        if any([show_box, show_count, show_top_and_bottom_n]) and not agg_func:
            raise ValueError(
                "show_box/show_count/show_top_and_bottom_n require agg_func"
            )

        if any([show_box, show_count, show_top_and_bottom_n]) and any(
            [
                plotly_kwargs.get("facet_col"),
                plotly_kwargs.get("facet_row"),
                plotly_kwargs.get("animation_frame"),
            ]
        ):
            raise ValueError(
                "Cannot use show_box/show_count/show_top_and_bottom_n with facet_col/facet_row/animation_frame"
            )
    ...
    def _validate_dimensions_requirements(self) -> None:
        ...
        if (
            dimensions_present
            and "x" not in self.plotly_kwargs
            and "y" not in self.plotly_kwargs
        ):
            raise ValueError(
                f"When using {', '.join(dimensions_present)}, at least one of x or y must be specified. "
                "Current values: x={self.plotly_kwargs.get('x')}, y={self.plotly_kwargs.get('y')}"
            )
    ...
    def _validate_agg_params(self) -> None:
        ...
        if (isinstance(agg_func, str) and agg_func not in self.VALID_AGG_FUNC) or (
            not isinstance(agg_func, str) and not callable(agg_func)
        ):
            raise ValueError(
                f'Invalid aggregation function "{agg_func}". Must be callable or one of: {self.VALID_AGG_FUNC}'
            )
        ...
        if len(grouping_cols) != len(set(grouping_cols)):
            raise ValueError(
                "When using agg_func x, y, color, facet_col, facet_row and animation_frame must be unique."
            )

        if not grouping_cols:
            raise ValueError(
                "When using agg_func, at least one of x, y, color, facet_col, facet_row, "
                "or animation_frame must be specified for grouping."
            )

        # Validate the agg_column is appropriate for the agg_func
        if agg_func not in ["count", "nunique"] and not pd.api.types.is_numeric_dtype(
            df[agg_column]
        ):
            raise ValueError(
                f'For non-numeric x and y columns, agg_func must be either "count" or "nunique", got "{agg_func}"'
            )
        if self.config.show_top_and_bottom_n:
    ...
    def _validate_datetime_columns(self) -> None:
        ...
        if isinstance(self.config.freq, (str, timedelta, DateOffset)):
            datetime_cols = []
            for dim in ["x", "y"]:
                if dim in self.plotly_kwargs and pd.api.types.is_datetime64_any_dtype(
                    df[self.plotly_kwargs[dim]]
                ):
                    datetime_cols.append(self.plotly_kwargs[dim])

            if not datetime_cols:
                raise ValueError(
                    f"Column x/y (current: x={x_col}, y={y_col}) is not datetime type but freq parameter is specified"
                )
        ...
        # For Freq in the form of a dictionary
            ...
            for col, freq in self.config.freq.items():
                if not pd.api.types.is_datetime64_any_dtype(df[col]):
                    raise ValueError(
                        f'Column "{col}" is not datetime type but freq parameter is specified'
                    )
    ...
    def _validate_trim_category_params(self) -> None:
        ...
        df = self.plotly_kwargs["data_frame"]
        show_top_and_bottom_n = self.config.show_top_and_bottom_n
        if show_top_and_bottom_n and (
            not isinstance(show_top_and_bottom_n, int) or show_top_and_bottom_n < 0
        ):
        ...
        if any(trim_params.values()) and not self.config.trim_top_n_agg_func:
            raise ValueError(
                "Top-N trimming requires specifying trim_top_n_agg_func (current: None)"
            )

        if self.config.trim_top_n_direction not in ["top", "bottom"]:
            raise ValueError(
                f'Invalid trim direction "{self.config.trim_top_n_direction}". Must be either "top" or "bottom"'
            )
        ...
        for dim, n in trim_params.items():
            if n is not None:
                ...
                if dim in self.plotly_kwargs and pd.api.types.is_numeric_dtype(
                    df[self.plotly_kwargs[dim]]
                ):
                    raise ValueError(
                        f'Cannot apply top-N trimming to numeric column "{self.plotly_kwargs[dim]}"'
                    )

    def _validate_norm_params(self) -> None:
        """Validate normalization parameters."""
        if self.config.norm_by and self.config.norm_by not in [
            "all",
            "x",
            "y",
            "color",
            ...
            raise ValueError(
                f'Invalid normalization target "{self.config.norm_by}". Must be one of: all or name of x, y, color'
            )
        ...
        if isinstance(self.config.freq, dict):
            for col, freq in self.config.freq.items():
                if col not in df.columns:
                    raise ValueError(
                        f'Date column "{col}" not found for time-based operations'
                    )
                if not self._is_valid_freq(test_df, freq):
                    raise ValueError(
                        f'Invalid frequency specification for column "{col}": {freq}.'
                        "See: https://pandas.pydata.org/pandas-docs/stable/user_guide/timeseries.html#offset-aliases"
                    )
        else:
            if not self._is_valid_freq(test_df, self.config.freq):
                raise ValueError(
                    f"Invalid frequency specification: {self.config.freq}."
                    "See: https://pandas.pydata.org/pandas-docs/stable/user_guide/timeseries.html#offset-aliases"
                )

    def _is_valid_freq(
        self, test_df: pd.DataFrame, freq: Union[str, timedelta, DateOffset]
    ) -> bool:
    ...
    def _validate_category_orders(self) -> None:
        ...
        has_string_orders = any(
            isinstance(order, str)
            for order in self.plotly_kwargs.get("category_orders", {}).values()
        )
        if has_string_orders and not self._has_numeric_column():
            raise ValueError(
                "No numeric column found for sorting operations. Required when using numeric sorting methods"
            )
    ...
    def _set_default_params_in_plotly_kwargs(self) -> None:
        ...
        if self.plot_type in ["line", "area"]:
            if self.plotly_kwargs.get("color"):
                self.plotly_kwargs.setdefault(
                    "color_discrete_sequence", colorway_for_line
                )
            self.plotly_kwargs.setdefault("line_shape", "spline")
        if self.config.agg_func in ["count", "nunique"]:
            self.plotly_kwargs.setdefault("hover_data", {}).update(
                {self.config.agg_column: ":.0f"}
            )
        if self.config.norm_by:
            self.plotly_kwargs.setdefault("hover_data", {}).update(
                {self.config.agg_column: ":.2f"}
            )
    ...
    def _calculate_normalization_factors(self) -> Optional[pd.Series]:
        ...
        if self.config.norm_by == "all":
            ...
            if not norm_col:
                raise ValueError(
                    f"Cannot normalize by {self.config.norm_by} - {self.config.norm_by} not specified"
                )

            # Calculate sum for each normalization group
            if agg_func:
                self.norm_factor = df.groupby(
                    [norm_col], observed=self.config.observed_for_groupby
                )[num_column].agg(agg_func)
            else:
                self.norm_factor = df.groupby(
                    [norm_col], observed=self.config.observed_for_groupby
                )[num_column].sum()
    ...
    def _trim_categories_by_top_n(self) -> None:
        ...
        for dim, n in trim_params.items():
            ...
            if dim not in plotly_kwargs:
                raise ValueError(
                    f"Cannot trim {dim} - not specified in plot parameters"
                )

            col = plotly_kwargs[dim]
            if not is_categorical_column(df_for_trim[col]):
                raise ValueError(
                    f'Top-N trimming cannot be applied to numeric column "{col}".'
                    "Only categorical columns can be trimmed"
                )

            ascending = self.config.trim_top_n_direction == "bottom"
            top_values = (
                df_for_trim.groupby(col, observed=self.config.observed_for_groupby)[
                    column_for_trim
                ]
    ...
    def _determine_column_for_operation(self, type: str) -> str:
        ...
        if self.config.agg_column:
            if not pd.api.types.is_numeric_dtype(
                df[self.config.agg_column]
            ) and self.config.agg_func not in ["count", "nunique"]:
                raise ValueError(
                    'For non-numeric x and y columns, agg_func must be either "count" or "nunique",'
                    f'got "{self.config.agg_func}"'
                )
        ...
        if x_col and y_col:
            ...
            if x_num and y_num:
                raise ValueError(
                    f"For both numeric x and y columns, {type} is not supported"
                )
            elif x_num:
                return x_col
            elif y_num:
                ...
                raise ValueError(
                    f"For both non numeric x and y columns, {type} is not supported"
                )
            ...
            raise ValueError(
                f"For non-numeric x and y columns, {type} is not supported"
            )
    ...
    def _apply_aggregation(self) -> None:
        ...
        if not group_cols:
            ...
            self.plotly_kwargs.setdefault("labels", {}).update(
                {new_agg_column: new_agg_column_label}
            )
            ...
            if plotly_kwargs.get("y") is not None:
                ...
                raise ValueError(
                    "x and y not specified when attempt grouping one categorical column"
                )

        # If we aggregate a categorical variable, then it ceases to be categorical
        # and we need to remove it from category_orders
        if self.plotly_kwargs.get(
            "category_orders", {}
        ) and agg_column in self.plotly_kwargs.get("category_orders", {}):
        ...
        # Perform aggregation
        agg_dict = {agg_column: agg_func, "count_for_show_group_size": "sum"}
        ...
        if datetime_cols:
            ...
            df_agg = (
                df.groupby(
                    groupers + other_cols, observed=self.config.observed_for_groupby
                )
                .agg(agg_dict)
                .reset_index()
            )
            ...
            df_agg = (
                df.groupby(group_cols, observed=self.config.observed_for_groupby)
                .agg(agg_dict)
                .reset_index()
            )

            if (
                plotly_kwargs.get("facet_col") or plotly_kwargs.get("facet_row")
            ) and plotly_kwargs.get("animation_frame"):
                other_cols = [col for col in group_cols if col not in datetime_cols]
                df_agg = self._restore_full_index(
                    df_agg,
                    date_cols=[],
                    group_cols=[
                        ...
                        if col != plotly_kwargs["animation_frame"]
                    ],
                )
        # Format count column
        if self.config.show_count:
    ...
    def _handle_datetime_columns(self, group_cols: List[str]) -> List[str]:
        ...
        df = self.plotly_kwargs["data_frame"]
        return [
            col for col in group_cols if pd.api.types.is_datetime64_any_dtype(df[col])
        ]

    def _filter_min_group_size(
        self, group_cols: List[str], num_column: str
    ) -> pd.DataFrame:
    ...
    def _restore_full_index(
        ...
        freq: Optional[
            Union[
                str, timedelta, DateOffset, Dict[str, Union[str, timedelta, DateOffset]]
            ]
        ] = None,
        ...
        for col in date_cols:
            current_freq = (
                self._get_freq_for_column(col) if isinstance(freq, dict) else freq
            )
            date_ranges.append(
                pd.date_range(df[col].min(), df[col].max(), freq=current_freq)
            )
        ...
        return (
            df.set_index(date_cols + group_cols)
    ...
    def _apply_normalization(self) -> None:
        ...
        if norm_by == "all":
            ...
            df = df.merge(
                norm_factor, left_on=self.plotly_kwargs[norm_by], right_index=True
            )
            ...
            if isinstance(order_spec, str):
                ...
                if order_spec not in valid_methods:
                    raise ValueError(
                        ...
                        ' Valid methods for {"aggregated" if agg_mode else'
                        ' "non-aggregated"} mode: {sorted(valid_methods)}'
                    )
                ...
                if "category" in order_spec:
                    ...
                    if agg_mode:
                        ...
                        column_for_sort = self._determine_column_for_operation(
                            "category orders"
                        )
                    ...
                    if not agg_mode:
                        ...
                        sorted_categories = (
                            df.groupby(col, observed=self.config.observed_for_groupby)[
                                column_for_sort
                            ]
                            .agg(agg_func)
                            .sort_values(ascending=ascending)
    ...
    def _create_figure(self) -> None:
        ...
        if (
            self.config.show_group_size
            and self.config.agg_func
            and (
    ...
    def _add_top_bottom_view(self) -> None:
        ...
        fig_subplots = make_subplots(
            rows=1, cols=2, horizontal_spacing=self.config.horizontal_spacing or 0.15
        )
        ...
        if self.plotly_kwargs.get("color"):
            ...
            self.plotly_kwargs.setdefault("category_orders", {}).update(
                {self.plotly_kwargs.get("color"): left_figure_color_order}
            )

        # Change categories for bottom figure
        if self.orientation == "h":
            self.plotly_kwargs.setdefault("category_orders", {})[
                self.plotly_kwargs.get("y")
            ] = "ascending"
        else:
            self.plotly_kwargs.setdefault("category_orders", {})[
                self.plotly_kwargs.get("x")
            ] = "ascending"
        ...
        # Combine plots
        self._combine_subplots(
            fig_subplots,
            left_figure,
            right_figure,
            left_axis_category_order,
            right_axis_category_order,
        )
    ...
    def _add_count_view(self) -> None:
        ...
        if kwargs_count.get("color"):
            left_figure_color_order = self._get_color_category_order(left_figure)
            kwargs_count.setdefault("category_orders", {}).update(
                {kwargs_count.get("color"): left_figure_color_order}
            )
        if self.COUNT_COLUMN not in kwargs_count["data_frame"]:
            raise ValueError(
                f"Count column '{self.COUNT_COLUMN}' not found in DataFrame"
            )
    ...
    def _get_quantile_range(self):
        ...
        cat_col = (
            self.plotly_kwargs["y"]
            if num_col == self.plotly_kwargs["x"]
            else self.plotly_kwargs["x"]
        )
        group_cols = [cat_col]
        if self.plotly_kwargs.get("color"):
        ...
        quantiles = (
            self.original_data_frame.groupby(
                group_cols, observed=self.config.observed_for_groupby
            )[num_col]
    ...
    def _get_axis_category_order(self, fig: go.Figure):
        ...
        if axis_name not in fig.layout:
            raise AttributeError(
                f"{axis_name} not found in fig.layout in _get_axis_category_order"
            )
    ...
    def _combine_subplots(
        ...
        fig_left,
        fig_right,
        left_axis_category_order=None,
        right_axis_category_order=None,
    ):
        ...
        if self.orientation == "h":
            ...
            subplots.update_yaxes(
                categoryorder="array",
                categoryarray=left_axis_category_order,
                row=1,
                col=1,
            )
            subplots.update_yaxes(
                categoryorder="array",
                categoryarray=right_axis_category_order,
                row=1,
                col=2,
            )
        else:
        ...
        # Update axis labels
        subplots.update_xaxes(title_text=xaxis_title_text_left, row=1, col=1)
        subplots.update_xaxes(title_text=xaxis_title_text_right, row=1, col=2)
        subplots.update_yaxes(title_text=yaxis_title_text_left, row=1, col=1)
        if self.orientation == "v" and self.config.show_count:
            subplots.update_yaxes(title_text=yaxis_title_text_right, row=1, col=2)
    ...
    def _apply_final_styling(self) -> None:
        ...
        if self.plotly_kwargs.get("color"):
            ...
            if self.plotly_kwargs.get("facet_col"):
                ...
                self.figure.update_layout(margin=dict(t=70), title_y=0.97)
            if not self.config.show_legend_title:
                update_config.update({"legend_title_text": ""})
            if self.plot_type in ["line", "area"]:
